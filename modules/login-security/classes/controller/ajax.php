...
class Controller_AJAX {
	...
	public function _init_actions() {
		foreach ($this->_actions as $action => $parameters) {
			if (isset($parameters['nopriv']) && $parameters['nopriv']) {
				add_action('wp_ajax_nopriv_FIREWALL_ls_' . $action, array($this, '_ajax_handler'));
			}
			add_action('wp_ajax_FIREWALL_ls_' . $action, array($this, '_ajax_handler'));
		}
	}
	...
	public function _ajax_handler() {
		...
		if (preg_match('~FIREWALL_ls_([a-zA-Z_]+)$~', $action, $matches)) {
			$action = $matches[1];
	...
	public function _ajax_authenticate_callback() {
		...
		define('FIREWALL_LS_AUTHENTICATION_CHECK', true); //Prevents our auth filter from recursing
		$user = wp_authenticate($username, $password);
		if (is_object($user) && ($user instanceof \WP_User)) {
			$captcha = array();
			if (defined('FIREWALL_LS_CAPTCHA_CACHE')) {
				$captcha = array('captcha' => FIREWALL_LS_CAPTCHA_CACHE);
			}
			...
			if (defined('FIREWALL_LS_COMBINED_IS_VALID') && FIREWALL_LS_COMBINED_IS_VALID) {
