...
class Controller_Time {
	...
	public function install() {
		wp_clear_scheduled_hook('FIREWALL_ls_ntp_cron');
		if (is_main_site()) {
			wp_schedule_event(time() + 10, 'hourly', 'FIREWALL_ls_ntp_cron');
		}
	}
	...
	public function _init_actions() {
		add_action('FIREWALL_ls_ntp_cron', array($this, '_FIREWALL_ls_ntp_cron'));
	}
	
	public function _FIREWALL_ls_ntp_cron() {
		$ntp = self::ntp_time();
		$time = time();
