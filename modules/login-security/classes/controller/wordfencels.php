...
class Controller_WordfenceLS {
	const VERSION_KEY = 'FIREWALL_ls_version';
	...
	protected function _init_actions() {
		register_activation_hook(FIREWALL_LS_FCPATH, array($this, '_install_plugin'));
		register_deactivation_hook(FIREWALL_LS_FCPATH, array($this, '_uninstall_plugin'));
		...
		if (!$versionInOptions || version_compare(FIREWALL_LS_VERSION, $versionInOptions, '>')) { //Either there is no version in options or the version in options is greater and we need to run the upgrade
			$this->_install();
		}
		...
		$useSubmenu = FIREWALL_LS_FROM_CORE;
	...
	public function _admin_init() {
		if (FIREWALL_LS_FROM_CORE) {
			...
			$link = __('Login Security Options', 'twodayssss');;
...
END
	...
	protected function _install() {
		...
		if (is_multisite() && function_exists('update_network_option')) {
			update_network_option(null, self::VERSION_KEY, FIREWALL_LS_VERSION); //In case we have a fatal error we don't want to keep running install.	
		}
		else {
			update_option(self::VERSION_KEY, FIREWALL_LS_VERSION); //In case we have a fatal error we don't want to keep running install.
		}
	...
	public function _login_enqueue_scripts() {
		...
		if ($useCAPTCHA || Controller_Users::shared()->any_2fa_active()) {
			...
			wp_enqueue_script('wordfence-ls-login', Model_Asset::js('login.js'), array('jquery'), FIREWALL_LS_VERSION);
			wp_enqueue_style('wordfence-ls-login', Model_Asset::css('login.css'), array(), FIREWALL_LS_VERSION);
	...
	public function _admin_enqueue_scripts() {
		if (isset($_GET['page']) && $_GET['page'] == 'WFLS') {
			wp_enqueue_script('wordfence-ls-jquery.qrcode', Model_Asset::js('jquery.qrcode.min.js'), array('jquery'), FIREWALL_LS_VERSION);
			wp_enqueue_script('wordfence-ls-jquery.tmpl', Model_Asset::js('jquery.tmpl.min.js'), array('jquery'), FIREWALL_LS_VERSION);
			wp_enqueue_script('wordfence-ls-jquery.colorbox', Model_Asset::js('jquery.colorbox.min.js'), array('jquery'), FIREWALL_LS_VERSION);
			if (Controller_Permissions::shared()->can_manage_settings()) { 
				wp_enqueue_script('wordfence-ls-jquery-ui.timepicker', Model_Asset::js('jquery-ui-timepicker-addon.js'), array('jquery', 'jquery-ui-datepicker', 'jquery-ui-slider'), FIREWALL_LS_VERSION);
				
				wp_enqueue_style('wordfence-ls-jquery-ui-css', Model_Asset::css('jquery-ui.min.css'), array(), FIREWALL_LS_VERSION);
				wp_enqueue_style('wordfence-ls-jquery-ui-css.structure', Model_Asset::css('jquery-ui.structure.min.css'), array(), FIREWALL_LS_VERSION);
				wp_enqueue_style('wordfence-ls-jquery-ui-css.theme', Model_Asset::css('jquery-ui.theme.min.css'), array(), FIREWALL_LS_VERSION);
				wp_enqueue_style('wordfence-ls-jquery-ui-css.timepicker', Model_Asset::css('jquery-ui-timepicker-addon.css'), array(), FIREWALL_LS_VERSION);
			}
			wp_enqueue_script('wordfence-ls-admin', Model_Asset::js('admin.js'), array('jquery'), FIREWALL_LS_VERSION);
			if (!FIREWALL_LS_FROM_CORE) {
				wp_register_script('chart-js', Model_Asset::js('Chart.bundle.min.js'), array('jquery'), '2.4.0');
				wp_register_script('wordfence-select2-js', Model_Asset::js('wfselect2.min.js'), array('jquery'), FIREWALL_LS_VERSION);
				wp_register_style('wordfence-select2-css', Model_Asset::css('wfselect2.min.css'), array(), FIREWALL_LS_VERSION);
			}
			wp_enqueue_script('chart-js');
			wp_enqueue_script('wordfence-select2-js');
			wp_enqueue_style('wordfence-select2-css');
			wp_enqueue_style('wordfence-ls-admin', Model_Asset::css('admin.css'), array(), FIREWALL_LS_VERSION);
			wp_enqueue_style('wordfence-ls-colorbox', Model_Asset::css('colorbox.css'), array(), FIREWALL_LS_VERSION);
			wp_enqueue_style('wordfence-ls-ionicons', Model_Asset::css('ionicons.css'), array(), FIREWALL_LS_VERSION);
			if (!FIREWALL_LS_FROM_CORE) { wp_enqueue_style('wordfence-ls-font-awesome', Model_Asset::css('font-awesome.css'), array(), FIREWALL_LS_VERSION); }
			wp_localize_script('wordfence-ls-admin', 'WFLSVars', array(
				'ajaxurl' => admin_url('admin-ajax.php'),
				'nonce' => wp_create_nonce('wp-ajax'),
				'modalTemplate' => Model_View::create('common/modal-prompt', array('title' => '${title}', 'message' => '${message}', 'primaryButton' => array('id' => 'wfls-generic-modal-close', 'label' => __('Close', 'twodayssss'), 'link' => '#')))->render(),
				'tokenInvalidTemplate' => Model_View::create('common/modal-prompt', array('title' => '${title}', 'message' => '${message}', 'primaryButton' => array('id' => 'wfls-token-invalid-modal-reload', 'label' => __('Reload', 'twodayssss'), 'link' => '#')))->render(),
				'modalHTMLTemplate' => Model_View::create('common/modal-prompt', array('title' => '${title}', 'message' => '{{html message}}', 'primaryButton' => array('id' => 'wfls-generic-modal-close', 'label' => __('Close', 'twodayssss'), 'link' => '#')))->render(),
			));
		}
		else {
			wp_enqueue_style('wordfence-ls-admin-global', Model_Asset::css('admin-global.css'), array(), FIREWALL_LS_VERSION);
		}
		
		if (Controller_Notices::shared()->has_notice(wp_get_current_user())) {
			wp_enqueue_script('wordfence-ls-admin-global', Model_Asset::js('admin-global.js'), array('jquery'), FIREWALL_LS_VERSION);
	...
	public function _authenticate($user, $username, $password) {
		...
		if (!empty($username)) { //Login attempt, not just a wp-login.php page load
			...
			$requireCAPTCHA = apply_filters('FIREWALL_ls_require_captcha', $requireCAPTCHA);
			...
			if (is_object($user) && $user instanceof \WP_User) {
				if (Controller_Users::shared()->has_2fa_active($user)) { //CAPTCHA enforcement skipped for users with 2FA active
					...
					if ($encryptedNonce) {
						...
						if (!defined('FIREWALL_LS_CAPTCHA_CACHE')) { define('FIREWALL_LS_CAPTCHA_CACHE', (string) $jwt); }
					}
				...
				if ($requireCAPTCHA && $performVerification) {
					...
					if ($encrypted) {
						$jwt = new Model_JWT(array('user' => $encrypted), Controller_Time::time() + 60 * FIREWALL_LS_EMAIL_VALIDITY_DURATION_MINUTES);
		 ...
		 * that if so.
		 */
		if (!defined('FIREWALL_LS_CHECKING_COMBINED') && (!isset($_POST['wfls-token']) || !is_string($_POST['wfls-token'])) && (!is_object($user) || !($user instanceof \WP_User))) {
			...
			if (isset($revisedPassword)) {
				define('FIREWALL_LS_CHECKING_COMBINED', true); //Avoid recursing into this block
				if (!defined('FIREWALL_LS_AUTHENTICATION_CHECK')) { define('FIREWALL_LS_AUTHENTICATION_CHECK', true); }
				...
				if (is_object($revisedUser) && ($revisedUser instanceof \WP_User) && Controller_TOTP::shared()->validate_2fa($revisedUser, $code)) {
					define('FIREWALL_LS_COMBINED_IS_VALID', true); //AJAX call will use this to generate a different JWT that authenticates for the account _and_ code
					return $revisedUser;
				}
			}
		}
		...
		if (defined('FIREWALL_LS_AUTHENTICATION_CHECK') && FIREWALL_LS_AUTHENTICATION_CHECK) { //Checking for the purpose of prompting for 2FA, don't enforce it here -- AJAX calls will halt here, POST will continue
			return $user;
		}
	...
	public function _admin_menu() {
		...
		$useSubmenu = FIREWALL_LS_FROM_CORE && current_user_can('activate_plugins');
		if (is_multisite() && !is_network_admin()) {
			$useSubmenu = false;
		...
		if ($useSubmenu) {
			add_submenu_page('twodayssss', __('Login Security', 'wordfence-2fa'), __('Login Security', 'wordfence-2fa'), Controller_Permissions::CAP_ACTIVATE_2FA_SELF, 'WFLS', array($this, '_menu'));
		}
		else {
	...
	public function _menu() {
		...
		$sections = array(
			array(
				...
				'title' => new Model_Title('manage', __('Two-Factor Authentication', 'wordfence-2fa'), Controller_Support::supportURL(Controller_Support::ITEM_MODULE_LOGIN_SECURITY_2FA), new Model_HTML(__('Learn more<span class="wfls-hidden-xs"> about Two-Factor Authentication</span>', 'twodayssss'))),
		...
		if ($administrator) {
			$sections[] = array(
				...
				'title' => new Model_Title('settings', __('Login Security Settings', 'wordfence-2fa'), Controller_Support::supportURL(Controller_Support::ITEM_MODULE_LOGIN_SECURITY), new Model_HTML(__('Learn more<span class="wfls-hidden-xs"> about Login Security</span>', 'twodayssss'))),
