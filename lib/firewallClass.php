...
if (version_compare(phpversion(), '5.3', '>=')) {
	...
	define('FIREWALL_USE_LEGACY_2FA', wfCredentialsController::useLegacy2FA());
	$wfCoreLoading = true;
...
class firewall {
	...
	public static $FIREWALL_wp_version = false;
	/**
	 * @var WP_Error
	 */
	...
	public static function installPlugin(){
		...
		update_option('tfActivated', 1);
		
		if (defined('FIREWALL_LS_FROM_CORE') && FIREWALL_LS_FROM_CORE) {
	...
	public static function uninstallPlugin(){
		...
		do_action('FIREWALL_security_event', 'wordfenceDeactivated', array(
			'username' => $username,
			'ip' => wfUtils::getIP(),
		), $alertCallback);
		...
		//Used by MU code below
		update_option('tfActivated', 0);
		wp_clear_scheduled_hook('FIREWALL_daily_cron');
		wp_clear_scheduled_hook('FIREWALL_hourly_cron');
		wp_clear_scheduled_hook('FIREWALL_daily_autoUpdate');

		//Remove old legacy cron job if it exists
		wp_clear_scheduled_hook('FIREWALL_scheduled_scan');
		...
		if(wfConfig::get('deleteTablesOnDeact')){
			...
			$schema = new wfSchema();
			$schema->dropAll();
			foreach(array('FIREWALL_VERSION', 'tfActivated', wfSchema::TABLE_CASE_OPTION) as $opt) {
	...
	public static function dailyCron() {
		...
		$wfdb->queryWrite("delete from {$table_wfLocs} where ctime < unix_timestamp() - %d", FIREWALL_MAX_IPLOC_AGE);
	...
	public static function _scheduleRefreshUpdateNotification($upgrader, $options) {
		...
		if (is_array($options) && isset($options['type']) && $options['type'] == 'core') {
			$defer = true;
			set_site_transient('FIREWALL_updating_notifications', true, 600);
		}
		
		if ($defer) {
			wp_schedule_single_event(time(), 'FIREWALL_refreshUpdateNotification');
		}
		else {
	...
	public static function _refreshUpdateNotification($report = null, $useCachedValued = false) {
		...
		wp_schedule_single_event(time(), 'FIREWALL_completeCoreUpdateNotification');
	}
	public static function _completeCoreUpdateNotification() {
		...
		delete_site_transient('FIREWALL_updating_notifications');
	...
	public static function runInstall(){
		...
		$previous_version = ((is_multisite() && function_exists('get_network_option')) ? get_network_option(null, 'FIREWALL_VERSION', '0.0.0') : get_option('FIREWALL_VERSION', '0.0.0'));
		if (is_multisite() && function_exists('update_network_option')) {
			update_network_option(null, 'FIREWALL_VERSION', FIREWALL_VERSION); //In case we have a fatal error we don't want to keep running install.	
		}
		else {
			update_option('FIREWALL_VERSION', FIREWALL_VERSION); //In case we have a fatal error we don't want to keep running install.
		}
...
SQL
		...
		if (!$hasAutoload) {
			...
			$wpdb->query("UPDATE {$configTable} SET autoload = 'no' WHERE name = 'wfsd_engine' OR name LIKE 'FIREWALL_chunked_%'");
		}
		...
		if (is_main_site()) {
			...
			wp_schedule_event(time() + 15, 'daily', 'FIREWALL_daily_cron'); //'daily'
			wp_schedule_event(time() + 15, 'hourly', 'FIREWALL_hourly_cron');
		}
...
END AS `expiration` FROM `{$oldBlocksTable}`", wfBlock::TYPE_WFSN_TEMPORARY, wfBlock::TYPE_RATE_BLOCK, wfBlock::TYPE_IP_AUTOMATIC_PERMANENT, wfConfig::get('blockedTime'));
			...
			if ($countries) {
				...
				wfBlock::createCountry(__('Automatically generated from previous country blocking settings', 'twodayssss'), wfConfig::get('cbl_loginFormBlocked', false), wfConfig::get('cbl_restOfSiteBlocked', false), $countries);
			}
...
<p><a href="javascript:WFAD.enableAllOptionsPage();" class="wf-btn wf-btn-primary wf-btn-callout-subtle">Enable "All Options" Page</a></p>', 'wfplugin_devalloptions');
		...
		if (!wp_next_scheduled('FIREWALL_VERSION_check')) {
			wp_schedule_single_event(time(), 'FIREWALL_VERSION_check');
		}
	...
	public static function install_actions(){
		register_activation_hook(TF_FCPATH, 'wordfence::installPlugin');
		register_deactivation_hook(TF_FCPATH, 'wordfence::uninstallPlugin');

		$versionInOptions = ((is_multisite() && function_exists('get_network_option')) ? get_network_option(null, 'FIREWALL_VERSION', false) : get_option('FIREWALL_VERSION', false));
		if( (! $versionInOptions) || version_compare(FIREWALL_VERSION, $versionInOptions, '>')){
		...
		if(defined('MULTISITE') && MULTISITE === true){
			global $blog_id;
			if($blog_id == 1 && get_option('tfActivated') != 1){ return; } //Because the plugin is active once installed, even before it's network activated, for site 1 (WordPress team, why?!)
		}
		//User may be logged in or not, so register both handlers
		add_action('wp_ajax_nopriv_FIREWALL_lh', 'wordfence::ajax_lh_callback');
		add_action('wp_ajax_nopriv_FIREWALL_doScan', 'wordfence::ajax_doScan_callback');
		add_action('wp_ajax_nopriv_FIREWALL_testAjax', 'wordfence::ajax_testAjax_callback');
		if(wfUtils::hasLoginCookie()){ //may be logged in. Fast way to check. These aren't secure functions, this is just a perf optimization, along with every other use of hasLoginCookie()
			add_action('wp_ajax_FIREWALL_lh', 'wordfence::ajax_lh_callback');
			add_action('wp_ajax_FIREWALL_doScan', 'wordfence::ajax_doScan_callback');
			add_action('wp_ajax_FIREWALL_testAjax', 'wordfence::ajax_testAjax_callback');
		...
		add_action('wp_ajax_FIREWALL_wafStatus', 'wordfence::ajax_wafStatus_callback');
		add_action('wp_ajax_nopriv_FIREWALL_wafStatus', 'wordfence::ajax_wafStatus_callback');
		
		add_action('wp_ajax_nopriv_FIREWALL_remoteVerifySwitchTo2FANew', 'wordfence::ajax_remoteVerifySwitchTo2FANew_callback');

		add_action('FIREWALL_start_scheduled_scan', 'wordfence::wordfenceStartScheduledScan');
		add_action('FIREWALL_daily_cron', 'wordfence::dailyCron');
		add_action('FIREWALL_daily_autoUpdate', 'wfConfig::autoUpdate');
		add_action('FIREWALL_hourly_cron', 'wordfence::hourlyCron');
		add_action('FIREWALL_VERSION_check', array(wfVersionCheckController::shared(), 'checkVersionsAndWarn'));
		...
		add_action('FIREWALL_email_activity_report', array('wfActivityReport', 'executeCronJob'));
		...
		if (self::hasGDLimitLoginsMUPlugin()) {
			add_action('login_errors', array('twodayssss', 'fixGDLimitLoginsErrors'), 11);
		}
		...
		add_action('FIREWALL_refreshUpdateNotification', 'wordfence::_refreshUpdateNotification', 99, 0);
		add_action('FIREWALL_completeCoreUpdateNotification', 'wordfence::_completeCoreUpdateNotification', 99, 0);
		...
		add_action('FIREWALL_processAttackData', 'wordfence::processAttackData');
		if (!empty($_GET['FIREWALL_syncAttackData']) && get_site_option('FIREWALL_syncingAttackData') <= time() - 60 && get_site_option('FIREWALL_lastSyncAttackData', 0) < time() - 4) {
			@ignore_user_abort(true);
			update_site_option('FIREWALL_syncingAttackData', time());
			header('Content-Type: text/javascript');
			define('FIREWALL_SYNCING_ATTACK_DATA', true);
			add_action('init', 'wordfence::syncAttackData', 10, 0);
			add_filter('woocommerce_unforce_ssl_checkout', '__return_false');
		}
		
		add_action('FIREWALL_batchReportBlockedAttempts', 'wordfence::wfsnBatchReportBlockedAttempts');
		add_action('FIREWALL_batchReportFailedAttempts', 'wordfence::wfsnBatchReportFailedAttempts');
		...
		if (wfCentral::isConnected()) {
			add_action('FIREWALL_security_event', 'wfCentral::sendSecurityEvent', 10, 3);
		} else {
			add_action('FIREWALL_security_event', 'wfCentral::sendAlertCallback', 10, 3);
		}
	}
	...
	public static function enqueueAJAXWatcher() {
		...
		if (wfUtils::isAdmin() && !$wafDisabled) {
			wp_enqueue_style('wordfenceAJAXcss', wfUtils::getBaseURL() . wfUtils::versionedAsset('css/wordfenceBox.css'), '', FIREWALL_VERSION);
			wp_enqueue_script('wordfenceAJAXjs', wfUtils::getBaseURL() . wfUtils::versionedAsset('js/admin.ajaxWatcher.js'), array('jquery'), FIREWALL_VERSION);
	...
	public static function enqueueDashboard() {
		if (wfUtils::isAdmin()) {
			wp_enqueue_style('wf-adminbar', wfUtils::getBaseURL() . wfUtils::versionedAsset('css/wf-adminbar.css'), '', FIREWALL_VERSION);
			wp_enqueue_script('wordfenceDashboardjs', wfUtils::getBaseURL() . wfUtils::versionedAsset('js/wfdashboard.js'), array('jquery'), FIREWALL_VERSION);
			if (wfConfig::get('showAdminBarMenu')) {
				wp_enqueue_script('wordfencePopoverjs', wfUtils::getBaseURL() . wfUtils::versionedAsset('js/wfpopover.js'), array('jquery'), FIREWALL_VERSION);
	...
	public static function ajaxReceiver(){
		...
		//func is e.g. FIREWALL_ticker so need to munge it
		$func = str_replace('FIREWALL_', '', $func);
	...
	public static function ajax_switchTo2FANew_callback() {
		...
		if ($migrate && is_array($twoFactorUsers) && !empty($twoFactorUsers)) {
			...
			$api = new wfAPI(wfConfig::get('apiKey'), wfUtils::getWPVersion());
			try {
				$response = $api->call('twoFactorTOTP_migrate', array(), array('migrateids' => json_encode($authenticatorActive), 'nonce' => $nonce, 'verifyurl' => add_query_arg(array('action' => 'FIREWALL_remoteVerifySwitchTo2FANew'), admin_url('admin-ajax.php'))));
				/*
	...
	public static function validatePassword($errors, $userData) {
		...
		if ($enforceStrongPasswds && !wordfence::isStrongPasswd($password, $username)) {
			$errors->add('pass', __('Please choose a stronger password. Try including numbers, symbols, and a mix of upper and lowercase letters and remove common words.', 'twodayssss'));
			return $errors;
		}
		
		$twoFactorUsers = wfConfig::get_ser('twoFactorUsers', array());
		if (preg_match(self::$passwordCodePattern, $password) && is_array($twoFactorUsers) && count($twoFactorUsers) > 0) {
			$errors->add('pass', __('Passwords containing a space followed by "wf" without quotes are not allowed.', 'twodayssss'));
			return $errors;
		}
		...
		if ($enforceBreachedPasswds && wfCredentialsController::isLeakedPassword($username, $password)) {
			$errors->add('pass', sprintf(__('Please choose a different password. The password you are using exists on lists of passwords leaked in data breaches. Attackers use such lists to break into sites and install malicious code. <a href="%s">Learn More</a>', 'twodayssss'), wfSupportController::esc_supportURL(wfSupportController::ITEM_USING_BREACH_PASSWORD)));
			return $errors;
		}
	...
	public static function lostPasswordPost() {
		...
		if($user){
			...
			do_action('FIREWALL_security_event', 'lostPasswdForm', array(
	...
	public static function lockOutIP($IP, $reason) {
		...
		do_action('FIREWALL_security_event', 'loginLockout', array(
			'ip'       => $IP,
			'reason'   => $reason,
	...
	public static function veryFirstAction() {
		...
		if($wfFunc == 'unlockEmail'){
			...
			if (wfUtils::isAdmin()) {
				...
				if (isset($_POST['license'])) {
					$nonceValid = wp_verify_nonce(@$_POST['nonce'], 'wf-form');
					if (!$nonceValid) {
						die(__('Sorry but your browser sent an invalid security token when trying to use this form.', 'twodayssss'));
					}
					
					$changes = array('apiKey' => $_POST['license']);
					$errors = wfConfig::validate($changes);
					if ($errors !== true) {
						$error = __('An error occurred while saving the license.', 'twodayssss');
						if (count($errors) == 1) {
							$error = sprintf(__('An error occurred while saving the license: %s', 'twodayssss'), $errors[0]['error']);
						}
					...
					try {
						...
						echo wfView::create('common/license', array(
							'state' => 'bad',
							'error' => sprintf(__('An error occurred while saving the license: %s', 'twodayssss'), $e->getMessage()),
						))->render();
						exit();
					}
				}
		...
		if (is_main_site() && wfUtils::isAdmin()) {
			if (wp_next_scheduled('FIREWALL_daily_cron') === false) {
				wp_schedule_event(time() + 600, 'daily', 'FIREWALL_daily_cron');
				wordfence::status(2, 'info', "Rescheduled missing daily cron");
			}
			
			if (wp_next_scheduled('FIREWALL_hourly_cron') === false) {
				wp_schedule_event(time() + 600, 'hourly', 'FIREWALL_hourly_cron');
		...
		if (!WFWAF_SUBDIRECTORY_INSTALL && $waf = wfWAF::getInstance()) {
			...
			try {
				...
				if (empty($_GET['FIREWALL_syncAttackData'])) {
					$table_wfHits = wfDB::networkTable('wfHits');
					...
					if (get_site_option('FIREWALL_lastSyncAttackData', 0) < time() - 4) {
						if ($waf->getStorageEngine()->hasNewerAttackData($lastAttackMicroseconds)) {
							if (get_site_option('FIREWALL_syncingAttackData') <= time() - 60) {
								// Could be the request to itself is not completing, add ajax to the head as a workaround
								$attempts = get_site_option('FIREWALL_syncAttackDataAttempts', 0);
								if ($attempts > 10) {
									...
									update_site_option('FIREWALL_syncAttackDataAttempts', ++$attempts);
									wp_remote_post(add_query_arg('FIREWALL_syncAttackData', microtime(true), home_url('/')), array(
										'timeout'   => 0.01,
										'blocking'  => false,
		...
		if(wfConfig::get('firewallEnabled')){
			...
			if (wfConfig::get('blockFakeBots')) {
				if (wfCrawl::isGooglebot() && !wfCrawl::isVerifiedGoogleCrawler()) {
					$reason = __('Fake Google crawler automatically blocked', 'twodayssss');
			...
			if (wfConfig::get('bannedURLs', false)) {
				...
				foreach ($URLs as $URL) {
					if (preg_match(wfUtils::patternToRegex($URL, ''), $_SERVER['REQUEST_URI'])) {
						$reason = __('Accessed a banned URL', 'twodayssss');
						wfBlock::createIP($reason, $IP, wfBlock::blockDuration(), time(), time(), 1, wfBlock::TYPE_IP_AUTOMATIC_TEMPORARY);
			...
			if (wfConfig::get('other_blockBadPOST') == '1' && $_SERVER['REQUEST_METHOD'] == 'POST' && empty($_SERVER['HTTP_USER_AGENT']) && empty($_SERVER['HTTP_REFERER'])) {
				$reason = __('POST received with blank user-agent and referer', 'twodayssss');
				wfBlock::createIP($reason, $IP, wfBlock::blockDuration(), time(), time(), 1, wfBlock::TYPE_IP_AUTOMATIC_TEMPORARY);
	...
	public static function loginAction($username){
		...
		if(wfUtils::isAdmin($userID)){
			...
			do_action('FIREWALL_security_event', $securityEvent, array(
				'username' => $username,
				'ip' => wfUtils::getIP(),
			), $alertCallback);
	...
	public static function registrationFilter($errors, $sanitizedLogin, $userEmail) {
		if (wfConfig::get('loginSec_blockAdminReg') && $sanitizedLogin == 'admin') {
			$errors->add('user_login_error', __('<strong>ERROR</strong>: You can\'t register using that username', 'twodayssss'));
		}
		return $errors;
	}
	...
	public static function jsonAPIAuthorFilter($response, $handler, $request) {
		...
		if (!current_user_can('list_users')) {
			...
			if (preg_match('~' . preg_quote($urlBase, '~') . '/*$~i', $route)) {
				...
				if (!defined('FIREWALL_REST_API_SUPPRESSED')) { define('FIREWALL_REST_API_SUPPRESSED', true); }
			}
	...
	public static function jsonAPIAdjustHeaders($response, $server, $request) {
		if (defined('FIREWALL_REST_API_SUPPRESSED')) {
	...
	public static function showTwoFactorField() {
        ...
        <input type=\"text\" size=\"6\" class=\"input\" id=\"FIREWALL_authFactor\" name=\"FIREWALL_authFactor\" autofocus></label>
        <input type=\"hidden\" id=\"FIREWALL_twoFactorUser\" name=\"FIREWALL_twoFactorUser\" value=\"" . $userID . "\">
        <input type=\"hidden\" id=\"FIREWALL_twoFactorNonce\" name=\"FIREWALL_twoFactorNonce\" value=\"" . $twoFactorNonce . "\">
    </p>";
	}
	private static function verifyTwoFactorIntermediateValues($userID, $twoFactorNonce) {
		...
		foreach ($twoFactorUsers as &$t) { //Ensure the two factor nonce hasn't expired
			if ($t[0] == $user->ID && $t[3] == 'activated') {
				if (isset($t[5]) && $t[5] == 'authenticator') { $graceTime = FIREWALL_TWO_FACTOR_GRACE_TIME_AUTHENTICATOR; }
				else { $graceTime = FIREWALL_TWO_FACTOR_GRACE_TIME_PHONE; }
	...
	public static function authenticateFilter($authUser, $username, $passwd) {
		...
		$twoFactorUsers = wfConfig::get_ser('twoFactorUsers', array());
		$userDat = (isset($_POST['FIREWALL_userDat']) ? $_POST['FIREWALL_userDat'] : false);
		...
		if ($checkTwoFactor) {
			...
			if (isset($_POST['FIREWALL_authFactor']) && $_POST['FIREWALL_authFactor'] && $twoFactorRecord) { //User authenticated with name and password, 2FA code ready to check
				$userID = $userDat->ID;
				...
				else if (isset($_POST['FIREWALL_twoFactorNonce'])) {
					$twoFactorNonce = preg_replace('/[^a-f0-9]/i', '', $_POST['FIREWALL_twoFactorNonce']);
					if (!self::verifyTwoFactorIntermediateValues($userID, $twoFactorNonce)) {
				...
				if ($usingBreachedPassword) {
					...
					wfAdminNoticeQueue::addAdminNotice(wfAdminNotice::SEVERITY_CRITICAL, sprintf(__('<strong>WARNING: </strong>The password you are using exists on lists of passwords leaked in data breaches. Attackers use such lists to break into sites and install malicious code. Please <a href="%s">change your password</a>. <a href="%s" target="_blank" rel="noopener noreferrer">Learn More</a>', 'twodayssss'), self_admin_url('profile.php'), wfSupportController::esc_supportURL(wfSupportController::ITEM_USING_BREACH_PASSWORD)), '2faBreachPassword', array($authUser->ID));
				}
				
				if (isset($twoFactorRecord[5])) { //New method TOTP
					$mode = $twoFactorRecord[5];
					$code = preg_replace('/[^a-f0-9]/i', '', $_POST['FIREWALL_authFactor']);
					...
					$authFactor = $_POST['FIREWALL_authFactor'];
				...
				if ($twoFactorRecord) {
					...
					if (wfCredentialsController::hasPreviousLoginFromIP($authUser, wfUtils::getIP())) {
						...
						do_action('FIREWALL_security_event', 'breachLogin', array(
							'username' => $username,
	...
	public static function checkSecurityNetwork($endpointType = null) {
		if (wfConfig::get('other_WFNet')) {
			...
			if ($maxBlockTime = self::wfsnIsBlocked($IP, 'brute', $endpointType)) {
				...
				$reason = __('Blocked by Wordfence Security Network', 'twodayssss');
				...
				if($blacklist = wfConfig::get('loginSec_userBlacklist')){
					...
					foreach($users as $user){
						if(strtolower($username) == strtolower($user)){
							...
							$reason = __('Blocked by login security setting', 'twodayssss');
	...
	public static function wfsnBatchReportBlockedAttempts() {
		...
		try {
			$response = wp_remote_post(FIREWALL_HACKATTEMPT_URL_SEC . 'multipleHackAttempts/?k=' . rawurlencode(wfConfig::get('apiKey')) . '&t=brute', array(
				'timeout' => 1,
				'user-agent' => "Wordfence.com UA " . (defined('FIREWALL_VERSION') ? FIREWALL_VERSION : '[Unknown version]'),
	...
	private static function wfsnScheduleBatchReportBlockedAttempts($timeToSend = null) {
		...
		if (!wp_next_scheduled('FIREWALL_batchReportBlockedAttempts')) {
			wp_schedule_single_event($timeToSend, 'FIREWALL_batchReportBlockedAttempts');
		}
		if ($notMainSite) {
	...
	private static function wfsnScheduleBatchReportFailedAttempts($timeToSend = null) {
		...
		if (!wp_next_scheduled('FIREWALL_batchReportFailedAttempts')) {
			wp_schedule_single_event($timeToSend, 'FIREWALL_batchReportFailedAttempts');
		}
		if ($notMainSite) {
			restore_current_blog();
		}
	...
	public static function wfsnIsBlocked($IP, $hitType, $endpointType = null) {
		...
		try {
			$result = wp_remote_get(FIREWALL_HACKATTEMPT_URL_SEC . 'hackAttempt/?k=' . rawurlencode(wfConfig::get('apiKey')) . 
	...
	public static function wfsnEndpointType() {
		...
		if (defined('XMLRPC_REQUEST') && XMLRPC_REQUEST) {
			...
			$type = 3;
			if (isset($_REQUEST['action']) && ($_REQUEST['action'] == 'FIREWALL_ls_authenticate' || $_REQUEST['action'] == 'nopriv_FIREWALL_ls_authenticate')) {
				$type = 301;
			}
		}
	...
	public static function authActionNew(&$username, &$passwd){ //As of php 5.4 we must denote passing by ref in the function definition, not the function call (as WordPress core does, which is a bug in WordPress).
		...
		if (isset($_POST['FIREWALL_twoFactorUser'])) { //Final stage of login -- get and verify 2fa code, make sure we load the appropriate user
			$userID = intval($_POST['FIREWALL_twoFactorUser']);
			$twoFactorNonce = preg_replace('/[^a-f0-9]/i', '', $_POST['FIREWALL_twoFactorNonce']);
			if (self::verifyTwoFactorIntermediateValues($userID, $twoFactorNonce)) {
				$user = get_user_by('ID', $userID);
				$username = $user->user_login;
				$passwd = $twoFactorNonce;
				$_POST['FIREWALL_userDat'] = $user;
				return;
			}
		}
		...
		$_POST['FIREWALL_userDat'] = $userDat;
		if(preg_match(self::$passwordCodePattern, $passwd, $matches)){
			$_POST['FIREWALL_authFactor'] = $matches[1];
	...
	public static function getWPFileContent($file, $cType, $cName, $cVersion){
		if ($cType == 'plugin') {
			...
			return array('errorMsg' => __('An invalid type was specified to get file.', 'twodayssss'));
		}
		...
		try {
			...
			throw new Exception(__('We could not fetch a core WordPress file from the Wordfence API.', 'twodayssss'));
		}
		catch (Exception $e) {
	...
	public static function ajax_sendTestEmail_callback(){
		...
		$result = false;
		if (count($emails)) {
			$result = wp_mail(implode(', ', $emails), __('Wordfence Test Email', 'twodayssss'), sprintf(__("This is a test email from %s.\nThe IP address that requested this was: %s", 'twodayssss'), site_url(), wfUtils::getIP()));
		}
	...
	public static function ajax_addTwoFactor_callback(){
		if(! wfConfig::get('isPaid')){
			return array('errorMsg' => __('Cellphone Sign-in is only available to paid members. <a href="https://www.wordfence.com/gnl1twoFac3/wordfence-signup/" target="_blank" rel="noopener noreferrer">Click here to upgrade now.</a>', 'twodayssss'));
		}
		...
		$user = get_user_by('login', $username);
		if(! $user){
			return array('errorMsg' => __("The username you specified does not exist.", 'twodayssss'));
		}
		...
		for ($i = 0; $i < sizeof($twoFactorUsers); $i++) {
			if ($twoFactorUsers[$i][0] == $user->ID) {
				return array('errorMsg' => __("The username you specified is already enabled.", 'twodayssss'));
			}
		}
		
		if ($mode != 'phone' && $mode != 'authenticator') {
			return array('errorMsg' => __("Unknown authentication mode.", 'twodayssss'));
		}
		
		if ($mode == 'phone') {
			if (!preg_match('/^\+\d[\d\-\(\)\s]+$/', $phone)) {
				return array('errorMsg' => __("The phone number you entered must start with a '+', then country code and then area code and number. For example, a number in the United States with country code '1' would look like this: +1-123-555-1234", 'twodayssss'));
			}
			$api = new wfAPI(wfConfig::get('apiKey'), wfUtils::getWPVersion());
			try {
				...
				return array('errorMsg' => sprintf(__("Could not contact Wordfence servers to generate a verification code: %s", 'twodayssss'), wp_kses($e->getMessage(), array())));
			}
			...
			if (isset($codeResult['ok']) && $codeResult['ok']) {
				...
				wordfence::status(4, 'info', sprintf(__("Could not gen verification code: %s", 'twodayssss'), var_export($codeResult, true)));
				return array('errorMsg' => __("We could not generate a verification code.", 'twodayssss'));
			}
		...
		return array('errorMsg' => __("Unknown two-factor authentication mode.", 'twodayssss'));
	}
	...
	public static function getNextScanStartTimestamp() {
		...
		foreach($cron as $key => $val){
			if(isset($val['FIREWALL_start_scheduled_scan'])){
				$nextTime = $key;
				break;
			}
		}
	...
	public static function ajax_sendActivityLog_callback(){
		...
		$content .= __('# Scan Issues', 'twodayssss') . "\n\n";
		...
		$content .= sprintf(__('## New Issues (%d total)', 'twodayssss'), $issueCounts['new']) . "\n\n";
		if (isset($issues['new']) && count($issues['new'])) {
			...
			$content .= __('No New Issues', 'twodayssss') . "\n\n";
		}
		
		$content .= str_repeat('-', 10);
		$content .= "\n\n";
		
		$content .= sprintf(__('## Ignored Issues (%d total)', 'twodayssss'), $issueCounts['ignoreP'] + $issueCounts['ignoreC']) . "\n\n";
		if (isset($issues['new']) && count($issues['new'])) {
			...
			$content .= __('No Ignored Issues', 'twodayssss') . "\n\n";
		}
		...
		if (count($emails)) {
			wp_mail(implode(', ', $emails), __('Wordfence Activity Log', 'twodayssss'), $content);
		}
		return array('ok' => 1);
	}
	...
	public static function ajax_misconfiguredHowGetIPsChoice_callback() {
		...
		if ($choice == 'yes') {
			...
			wfConfig::set('misconfiguredHowGetIPsChoice' . FIREWALL_VERSION, '1');
		}
		return array('ok' => 1);
	}
	...
	public static function _blocksAJAXReponse(&$hasCountryBlock = false, $offset = 0, $sortColumn = 'type', $sortDirection = 'ascending', $filter = '') {
		...
		if (empty($filter)) {
			$blocks = wfBlock::allBlocks(true, $types, $offset, FIREWALL_BLOCKED_IPS_PER_PAGE, $sortColumn, $sortDirection);
		}
		else {
			$blocks = wfBlock::filteredBlocks(true, $types, $offset, FIREWALL_BLOCKED_IPS_PER_PAGE, $sortColumn, $sortDirection, $filter);
		}
		...
		foreach ($blocks as $b) {
			...
			switch ($b->type) {
				...
				case wfBlock::TYPE_COUNTRY:
					require(TF_PATH . 'lib/wfBulkCountries.php'); /** @var array $wfBulkCountries */
					...
					if ($countries == array_keys($wfBulkCountries)) {
						$entry['detailDisplay'] = __('All Countries', 'twodayssss');
					}
					else if (count($countries) == 1) {
						$entry['detailDisplay'] = __('1 Country', 'twodayssss');
					}
					else {
						$entry['detailDisplay'] = sprintf(__('%d Countries', 'twodayssss'), count($countries));
					}
					
					if ($b->blockLogin && $b->blockSite) {
						$entry['detailDisplay'] .= ' (' . __('Entire Site', 'twodayssss') . ')';
					}
					else if ($b->blockLogin) {
						$entry['detailDisplay'] .= ' (' . __('Login Only', 'twodayssss') . ')';
					}
					else if ($b->blockSite) {
						$entry['detailDisplay'] .= ' (' . __('Site Except Login', 'twodayssss') . ')';
					}
				...
				case wfBlock::TYPE_PATTERN:
					...
					$components = array();
					if (!empty($b->ipRange)) { $components[] = __('IP Range', 'twodayssss') . ' - ' . $b->ipRange; }
					if (!empty($b->userAgent)) { $components[] = __('User Agent', 'twodayssss') . ' - ' . $b->userAgent; }
					if (!empty($b->referrer)) { $components[] = __('Referrer', 'twodayssss') . ' - ' . $b->referrer; }
					if (!empty($b->hostname)) { $components[] = __('Hostname', 'twodayssss') . ' - ' . $b->hostname; }
			...
			$entry['expirationDisplay'] = ($b->expiration == wfBlock::DURATION_FOREVER ? __('Permanent', 'twodayssss') : esc_html(wfUtils::formatLocalTime($dateFormat, $b->expiration)));
			$entry['blockCountSort'] = $b->blockedHits;
			$entry['blockCountDisplay'] = $b->blockedHits;
			$entry['lastAttemptSort'] = $b->lastAttempt;
			$entry['lastAttemptDisplay'] = ($b->lastAttempt == 0 ? __('Never', 'twodayssss') : esc_html(wfUtils::formatLocalTime($dateFormat, $b->lastAttempt)));
	...
	public static function ajax_createBlock_callback() {
		...
		if (!empty($_POST['payload']) && ($payload = json_decode(stripslashes($_POST['payload']), true)) !== false) {
			try {
				...
				return array(
					'error' => __('An error occurred while creating the block.', 'twodayssss'),
				);
			}
		}
		
		return array(
			'error' => __('No block parameters were provided.', 'twodayssss'),
		);
	}
	public static function ajax_deleteBlocks_callback() {
		...
		return array(
			'error' => __('No blocks were provided.', 'twodayssss'),
		);
	}
	...
	public static function ajax_installLicense_callback() {
		if (!empty($_POST['license'])) {
			...
			if (!preg_match('/^[a-fA-F0-9]+$/', $license)) {
				return array(
					'error' => __('The license key entered is not in a valid format. It must contain only numbers and the letters A-F.', 'twodayssss'),
				);
			}
			...
			if ($existingLicense != $license) { //Key changed, try activating
				...
				try {
					...
					if ($res['ok'] && isset($res['isPaid'])) {
						...
						return array(
							'error' => __('The license provided is already in use on another site.', 'twodayssss'),
						);
					}
					else {
						return array(
							'error' => __('The Wordfence activation server returned an unexpected response. Please try again.', 'twodayssss'),
						);
					}
				}
				catch (Exception $e) {
					return array(
						'error' => __('We received an error while trying to activate the license with the Wordfence servers: ', 'twodayssss') . wp_kses($e->getMessage(), array())
					);
				}
			}
			else {
		...
		return array(
			'error' => __('No license was provided to install.', 'twodayssss'),
		);
	}
	...
	public static function ajax_restoreDefaults_callback() {
		if (!empty($_POST['section'])) {
			if (wfConfig::restoreDefaults($_POST['section'])) {
				...
				return array(
					'error' => __('An unknown configuration section was provided.', 'twodayssss'),
				);
			}
		}
		
		return array(
			'error' => __('No configuration section was provided.', 'twodayssss'),
		);
	}
	public static function ajax_saveOptions_callback() {
		if (!empty($_POST['changes']) && ($changes = json_decode(stripslashes($_POST['changes']), true)) !== false) {
			try {
				...
				if ($errors !== true) {
					if (count($errors) == 1) {
						return array(
							'error' => sprintf(__('An error occurred while saving the configuration: %s', 'twodayssss'), $errors[0]['error']),
						);
					}
						...
						return array(
							'error' => sprintf(__('Errors occurred while saving the configuration: %s', 'twodayssss'), implode(', ', $compoundMessage)),
						);
					}
					
					return array(
						'error' => __('Errors occurred while saving the configuration.', 'twodayssss'),
					);
				}
				...
				return array(
					'error' => __('An error occurred while saving the configuration.', 'twodayssss'),
				);
			}
			catch (wfWAFStorageEngineMySQLiException $e) {
				return array(
		...
		return array(
			'error' => __('No configuration changes were provided to save.', 'twodayssss'),
		);
	}
...
HTACCESS;
	...
	public static function ajax_permBlockIP_callback(){
		$IP = $_POST['IP'];
		wfBlock::createIP(__('Manual permanent block by admin', 'twodayssss'), $IP, wfBlock::DURATION_FOREVER, time(), false, 0, wfBlock::TYPE_IP_MANUAL);
		return array('ok' => 1);
	}
	...
	public static function ajax_loadIssues_callback(){
		...
		$limit = isset($_POST['limit']) ? intval($_POST['limit']) : FIREWALL_SCAN_ISSUES_PER_PAGE;
		$ignoredOffset = isset($_POST['ignoredOffset']) ? intval($_POST['ignoredOffset']) : 0;
		$ignoredLimit = isset($_POST['ignoredLimit']) ? intval($_POST['ignoredLimit']) : FIREWALL_SCAN_ISSUES_PER_PAGE;
	...
	public static function ajax_ticker_callback() {
		...
		$events = array();
		if (get_site_option('FIREWALL_syncAttackDataAttempts') > 10) {
	...
	public static function ajax_activityLogUpdate_callback() {
		...
		$lastMessage = __('Idle', 'twodayssss');
		...
		if ($row) {
			...
			if ($scanLastCompletion) {
				$lastMessage = sprintf(__('Scan completed on %s', 'twodayssss'), wfUtils::formatLocalTime(get_option('date_format') . ' ' . get_option('time_format'), $scanLastCompletion));
			}
		}
			...
			$lastMessage = __('Last scan failed', 'twodayssss');
		}
		...
		if ($lastIssueUpdateTimestamp > $_POST['lastissuetime']) {
			$issues = wfIssues::shared()->getIssues(0, FIREWALL_SCAN_ISSUES_PER_PAGE, 0, FIREWALL_SCAN_ISSUES_PER_PAGE);
		}
		
		$timeLimit = intval(wfConfig::get('scan_maxDuration'));
		if ($timeLimit < 1) {
			$timeLimit = FIREWALL_DEFAULT_MAX_SCAN_TIME;
		}
		...
		switch ($scanFailed) {
			case wfIssues::SCAN_FAILED_TIMEOUT:
				...
				$scanFailedHTML = wfView::create('scanner/scan-failed', array(
					'messageHTML' => sprintf(__('The current scan looks like it has failed. Its last status update was <span id="wf-scan-failed-time-ago">%s</span> ago. You may continue to wait in case it resumes or stop and restart the scan. Some sites may need adjustments to run scans reliably.', 'twodayssss'), $scanFailedTiming) . ' <a href="' . wfSupportController::esc_supportURL(wfSupportController::ITEM_SCAN_FAILS) . '" target="_blank" rel="noopener noreferrer">' . __('Click here for steps you can try.', 'twodayssss') . '</a>',
					'buttonTitle' => __('Cancel Scan', 'twodayssss'), 
				))->render();
			...
			case wfIssues::SCAN_FAILED_GENERAL:
				$scanFailedHTML = wfView::create('scanner/scan-failed', array(
					'messageHTML' => __('The previous scan has failed. Some sites may need adjustments to run scans reliably.', 'twodayssss') . ' <a href="' . wfSupportController::esc_supportURL(wfSupportController::ITEM_SCAN_FAILS) . '" target="_blank" rel="noopener noreferrer">' . __('Click here for steps you can try.', 'twodayssss') . '</a>',
					'buttonTitle' => __('Close', 'twodayssss'),
				))->render();
				break;
			case wfIssues::SCAN_FAILED_DURATION_REACHED:
				$scanFailedHTML = wfView::create('scanner/scan-failed', array(
					'messageHTML' => sprintf(__('The previous scan has terminated because the time limit of %s was reached. This limit can be customized on the options page.', 'twodayssss'), wfUtils::makeDuration($timeLimit)) . ' <a href="' . wfSupportController::esc_supportURL(wfSupportController::ITEM_SCAN_OPTION_OVERALL_TIME_LIMIT) . '" target="_blank" rel="noopener noreferrer" class="wf-inline-help"><i class="wf-fa wf-fa-question-circle-o" aria-hidden="true"></i></a>',
					'buttonTitle' => __('Close', 'twodayssss'),
				))->render();
				break;
			case wfIssues::SCAN_FAILED_VERSION_CHANGE:
				$scanFailedHTML = wfView::create('scanner/scan-failed', array(
					'messageHTML' => __('The previous scan has terminated because we detected an update occurring during the scan.', 'twodayssss'),
					'buttonTitle' => __('Close', 'twodayssss'),
				))->render();
				break;
			...
			case wfIssues::SCAN_FAILED_CALLBACK_TEST_FAILED:
				$scanFailedHTML = wfView::create('scanner/scan-failed', array(
					'messageHTML' => __('The scan has failed to start. This is often because the site either cannot make outbound requests or is blocked from connecting to itself.', 'twodayssss') . ' <a href="' . wfSupportController::esc_supportURL(wfSupportController::ITEM_SCAN_FAILED_START) . '" target="_blank" rel="noopener noreferrer">' . __('Click here for steps you can try.', 'twodayssss') . '</a>',
					'buttonTitle' => __('Close', 'twodayssss'),
				))->render();
				break;
			case wfIssues::SCAN_FAILED_API_SSL_UNAVAILABLE:
				$scanFailedHTML = wfView::create('scanner/scan-failed', array(
					'messageHTML' => __('Scans are not functional because SSL is unavailable.', 'twodayssss'),
					'buttonTitle' => __('Close', 'twodayssss'),
				))->render();
				break;
			case wfIssues::SCAN_FAILED_API_CALL_FAILED:
				$scanFailedHTML = wfView::create('scanner/scan-failed', array(
					'messageHTML' => __('The scan has failed because we were unable to contact the Wordfence servers. Some sites may need adjustments to run scans reliably.', 'twodayssss') . ' <a href="' . wfSupportController::esc_supportURL(wfSupportController::ITEM_SCAN_FAILS) . '" target="_blank" rel="noopener noreferrer">' . __('Click here for steps you can try.', 'twodayssss') . '</a>',
			...
			case wfIssues::SCAN_FAILED_API_ERROR_RESPONSE:
				$scanFailedHTML = wfView::create('scanner/scan-failed', array(
					'messageHTML' => __('The scan has failed because we received an unexpected response from the Wordfence servers. This may be a temporary error, though some sites may need adjustments to run scans reliably.', 'twodayssss') . ' <a href="' . wfSupportController::esc_supportURL(wfSupportController::ITEM_SCAN_FAILS) . '" target="_blank" rel="noopener noreferrer">' . __('Click here for steps you can try.', 'twodayssss') . '</a>',
					'rawErrorHTML' => esc_html(wfConfig::get('lastScanCompleted'), ''),
					'buttonTitle' => __('Close', 'twodayssss'),
				))->render();
				break;
	...
	public static function ajax_bulkOperation_callback() {
		...
		if ($op == 'del' || $op == 'repair') {
				...
				foreach ($issues['new'] as $i) {
					if ($op == 'del' && @$i['data']['canDelete']) {
						...
						if ($localFile === ABSPATH . 'wp-config.php') {
							$errors[] = esc_html__('Deleting an infected wp-config.php file must be done outside of Wordfence. The wp-config.php file contains your database credentials, which you will need to restore normal site operations. Your site will NOT function once the wp-config.php file has been deleted.', 'twodayssss');
						}
							...
							$err = error_get_last();
							$errors[] = esc_html(sprintf(__('Could not delete file %s. Error was: %s', 'twodayssss'), wp_kses($file, array()), wp_kses(str_replace(ABSPATH, '{WordPress Root}/', $err['message']), array())));
						}
					}
						...
						if (is_array($result) && isset($result['errorMsg'])) {
							...
							$errors[] = esc_html(sprintf(__('We could not retrieve the original file of %s to do a repair.', 'twodayssss'), wp_kses($file, array())));
							continue;
						}
						
						if (preg_match('/\.\./', $file)) {
							$errors[] = sprintf(__('An invalid file %s was specified for repair.', 'twodayssss'), wp_kses($file, array()));
							continue;
						}
						...
						if (!$fh) {
							...
							if (preg_match('/Permission denied/i', $err['message'])) {
								$errMsg = esc_html(sprintf(__('You don\'t have permission to repair %s. You need to either fix the file manually using FTP or change the file permissions and ownership so that your web server has write access to repair the file.', 'twodayssss'), wp_kses($file, array())));
							}
							else {
								$errMsg = esc_html(sprintf(__('We could not write to %s. The error was: %s', 'twodayssss'), wp_kses($file, array()),  $err['message']));
							}
							$errors[] = $errMsg;
							continue;
						}
						...
						fclose($fh);
						if ($bytes < 1) {
							$errors[] = esc_html(sprintf(__('We could not write to %s. (%d bytes written) You may not have permission to modify files on your WordPress server.', 'twodayssss'), wp_kses($file, array()), $bytes));
							continue;
						}
			...
			if ($filesWorkedOn > 0 && count($errors) > 0) {
				$headMsg = esc_html($op == 'del' ? __('Deleted some files with errors', 'twodayssss') : __('Repaired some files with errors', 'twodayssss'));
				$bodyMsg = sprintf(esc_html($op == 'del' ? __('Deleted %d files but we encountered the following errors with other files: %s', 'twodayssss') : __('Repaired %d files but we encountered the following errors with other files: %s', 'twodayssss')), $filesWorkedOn, implode('<br>', $errors));
			}
			else if ($filesWorkedOn > 0) {
				$headMsg = sprintf(esc_html($op == 'del' ? __('Deleted %d files successfully', 'twodayssss') : __('Repaired %d files successfully', 'twodayssss')), $filesWorkedOn);
				$bodyMsg = sprintf(esc_html($op == 'del' ? __('Deleted %d files successfully. No errors were encountered.', 'twodayssss') : __('Repaired %d files successfully. No errors were encountered.', 'twodayssss')), $filesWorkedOn);
			}
			else if (count($errors) > 0) {
				$headMsg = esc_html($op == 'del' ? __('Could not delete files', 'twodayssss') : __('Could not repair files', 'twodayssss'));
				$bodyMsg = sprintf(esc_html($op == 'del' ? __('We could not delete any of the files you selected. We encountered the following errors: %s', 'twodayssss') : __('We could not repair any of the files you selected. We encountered the following errors: %s', 'twodayssss')),  implode('<br>', $errors));
			}
			else {
				$headMsg = esc_html__('Nothing done', 'twodayssss');
				$bodyMsg = esc_html($op == 'del' ? __('We didn\'t delete anything and no errors were found.', 'twodayssss') : __('We didn\'t repair anything and no errors were found.', 'twodayssss'));
			}
			...
			return array('errorMsg' => esc_html__('Invalid bulk operation selected', 'twodayssss'));
		}
	}
	public static function ajax_deleteFile_callback($issueID = null){
		...
		if(! $issue){
			return array('errorMsg' => __('Could not delete file because we could not find that issue.', 'twodayssss'));
		}
		if(! $issue['data']['file']){
			return array('errorMsg' => __('Could not delete file because that issue does not appear to be a file related issue.', 'twodayssss'));
		}
		$file = $issue['data']['file'];
		$localFile = ABSPATH . '/' . $file;
		$localFile = realpath($localFile);
		if(strpos($localFile, ABSPATH) !== 0){
			return array('errorMsg' => __('An invalid file was requested for deletion.', 'twodayssss'));
		}
		if ($localFile === ABSPATH . 'wp-config.php') {
			return array(
				'errorMsg' => __('Deleting an infected wp-config.php file must be done outside of Wordfence. The wp-config.php file contains your database credentials, which you will need to restore normal site operations. Your site will NOT function once the wp-config.php file has been deleted.', 'twodayssss')
			);
		}
	...
	private static function wfFunc_testtime(){
		...
		echo "Wordfence process duration benchmarking utility version " . FIREWALL_VERSION . ".\n";
	...
	private static function wfFunc_testmem(){
		...
		echo "Wordfence Memory benchmarking utility version " . FIREWALL_VERSION . ".\n";
	...
	public static function wfLogHumanHeader(){
		...
		if (self::$hitID) {
			$URL = home_url('/?FIREWALL_lh=1&hid=' . wfUtils::encrypt(self::$hitID));
...
HTML;
	...
	public static function wfFunc_view(){
		wfUtils::doNotCache();
		if (FIREWALL_DISABLE_FILE_VIEWER) {
			echo "File access blocked. (FIREWALL_DISABLE_FILE_VIEWER is true)";
			exit();
		}
	...
	public static function admin_init(){
		...
		foreach(array(
			...
			add_action('wp_ajax_FIREWALL_' . $func, 'wordfence::ajaxReceiver');
		}
		
		wp_register_script('chart-js', wfUtils::getBaseURL() . wfUtils::versionedAsset('js/Chart.bundle.min.js'), array('jquery'), '2.4.0');
		wp_register_script('wordfence-select2-js', wfUtils::getBaseURL() . wfUtils::versionedAsset('js/wfselect2.min.js'), array('jquery', 'jquery-ui-tooltip'), FIREWALL_VERSION);
		wp_register_style('wordfence-select2-css', wfUtils::getBaseURL() . wfUtils::versionedAsset('css/wfselect2.min.css'), array(), FIREWALL_VERSION);
		wp_register_style('wordfence-font-awesome-style', wfUtils::getBaseURL() . wfUtils::versionedAsset('css/wf-font-awesome.css'), '', FIREWALL_VERSION);
		...
		if (isset($_GET['page']) && (preg_match('/^Wordfence/', @$_GET['page']) || ($_GET['page'] == 'WFLS' && wfOnboardingController::shouldShowNewTour(wfOnboardingController::TOUR_LOGIN_SECURITY)))) {
			...
			wp_enqueue_style('wordfence-font', wfUtils::getBaseURL() . wfUtils::versionedAsset('css/wf-roboto-font.css'), '', FIREWALL_VERSION);
			wp_enqueue_style('wordfence-font-awesome-style');
			wp_enqueue_style('wordfence-main-style', wfUtils::getBaseURL() . wfUtils::versionedAsset('css/main.css'), '', FIREWALL_VERSION);
			wp_enqueue_style('wordfence-ionicons-style', wfUtils::getBaseURL() . wfUtils::versionedAsset('css/wf-ionicons.css'), '', FIREWALL_VERSION);
			wp_enqueue_style('wordfence-colorbox-style', wfUtils::getBaseURL() . wfUtils::versionedAsset('css/wf-colorbox.css'), '', FIREWALL_VERSION);

			wp_enqueue_script('json2');
			wp_enqueue_script('jquery-ui-core');
			wp_enqueue_script('jquery-ui-menu');
			wp_enqueue_script('jquery.wftmpl', wfUtils::getBaseURL() . wfUtils::versionedAsset('js/jquery.tmpl.min.js'), array('jquery'), FIREWALL_VERSION);
			wp_enqueue_script('jquery.wfcolorbox', wfUtils::getBaseURL() . wfUtils::versionedAsset('js/jquery.colorbox-min.js'), array('jquery'), FIREWALL_VERSION);
			wp_enqueue_script('jquery.wfdataTables', wfUtils::getBaseURL() . wfUtils::versionedAsset('js/jquery.dataTables.min.js'), array('jquery'), FIREWALL_VERSION);
			wp_enqueue_script('jquery.qrcode', wfUtils::getBaseURL() . wfUtils::versionedAsset('js/jquery.qrcode.min.js'), array('jquery'), FIREWALL_VERSION);
			//wp_enqueue_script('jquery.tools', wfUtils::getBaseURL() . wfUtils::versionedAsset('js/jquery.tools.min.js'), array('jquery'));
			wp_enqueue_script('wordfenceAdminjs', wfUtils::getBaseURL() . wfUtils::versionedAsset('js/admin.js'), array('jquery', 'jquery-ui-core', 'jquery-ui-menu'), FIREWALL_VERSION);
			wp_enqueue_script('wordfenceAdminExtjs', wfUtils::getBaseURL() . wfUtils::versionedAsset('js/wfglobal.js'), array('jquery'), FIREWALL_VERSION);
			wp_enqueue_script('wordfenceDropdownjs', wfUtils::getBaseURL() . wfUtils::versionedAsset('js/wfdropdown.js'), array('jquery'), FIREWALL_VERSION);
			self::setupAdminVars();
			...
			wp_enqueue_script('wordfenceAdminExtjs', wfUtils::getBaseURL() . wfUtils::versionedAsset('js/wfglobal.js'), array('jquery'), FIREWALL_VERSION);
			wp_enqueue_style('wordfence-font-awesome-style');
			wp_enqueue_style('wordfence-global-style', wfUtils::getBaseURL() . wfUtils::versionedAsset('css/wf-global.css'), '', FIREWALL_VERSION);
			self::setupAdminVars();
		}
	...
	private static function setupAdminVars(){
		...
		wp_localize_script('wordfenceAdminExtjs', 'WordfenceAdminVars', array(
			...
			'scanIssuesPerPage' => FIREWALL_SCAN_ISSUES_PER_PAGE,
			'allowsPausing' => wfConfig::get('liveActivityPauseEnabled'),
			'scanRunning' => wfScanner::shared()->isRunning() ? '1' : '0',
			'modalTemplate' => wfView::create('common/modal-prompt', array('title' => '${title}', 'message' => '${message}', 'primaryButton' => array('id' => 'wf-generic-modal-close', 'label' => __('Close', 'twodayssss'), 'link' => '#')))->render(),
			'tokenInvalidTemplate' => wfView::create('common/modal-prompt', array('title' => '${title}', 'message' => '${message}', 'primaryButton' => array('id' => 'wf-token-invalid-modal-reload', 'label' => __('Reload', 'twodayssss'), 'link' => '#')))->render(),
			'modalHTMLTemplate' => wfView::create('common/modal-prompt', array('title' => '${title}', 'message' => '{{html message}}', 'primaryButton' => array('id' => 'wf-generic-modal-close', 'label' => __('Close', 'twodayssss'), 'link' => '#')))->render(),
	...
	public static function wafConfigInaccessibleNotice() {
		...
		echo '<div id="wafConfigInaccessibleNotice" class="fade error"><p><strong>' . __('The Wordfence Web Application Firewall cannot run.', 'twodayssss') . '</strong> ' . sprintf('The configuration files are corrupt or inaccessible by the web server, which is preventing the WAF from functioning. Please verify the web server has permission to access the configuration files. You may also try to rebuild the configuration file by <a href="%s">clicking here</a>. It will automatically resume normal operation when it is fixed. <a class="wfhelp" target="_blank" rel="noopener noreferrer" href="%s"></a>', $wafMenuURL, wfSupportController::esc_supportURL(wfSupportController::ITEM_NOTICE_WAF_INACCESSIBLE_CONFIG)) . '</p></div>';
	}
	...
	public static function admin_menus(){
		...
		if ($firewall->firewallMode() != wfFirewall::FIREWALL_MODE_DISABLED) {
			try {
				...
				if ($lastChecked > 0 && $lastUpdated > 0 && $lastChecked < $threshold) {
					...
					$message = sprintf(__('The last rules update for the Wordfence Web Application Firewall was unsuccessful. The last successful update check was %s, so this site may be missing new rules added since then.', 'twodayssss'), wfUtils::formatLocalTime(get_option('date_format') . ' ' . get_option('time_format'), $lastChecked));
					
					if (!$firewall->isSubDirectoryInstallation()) {
						if ($nextUpdate < PHP_INT_MAX) {
							$message .= ' ' . sprintf(__('You may wait for the next automatic attempt at %s or try to <a href="%s">Manually Update</a> by clicking the "Manually Refresh Rules" button below the Rules list.', 'twodayssss'), wfUtils::formatLocalTime(get_option('date_format') . ' ' . get_option('time_format'), $nextUpdate), esc_url(network_admin_url('admin.php?page=WordfenceWAF&subpage=waf_options#wf-option-wafRules')));
						}
						else {
							$message .= ' ' . sprintf(__('You may wait for the next automatic attempt or try to <a href="%s">Manually Update</a> by clicking the "Manually Refresh Rules" button below the Rules list.', 'twodayssss'), esc_url(network_admin_url('admin.php?page=WordfenceWAF&subpage=waf_options#waf-rules-next-update')));
						}
					}
					else {
						if ($nextUpdate < PHP_INT_MAX) {
							$message .= ' ' . sprintf(__('You may wait for the next automatic attempt at %s or log into the parent site to manually update by clicking the "Manually Refresh Rules" button below the Rules list.', 'twodayssss'), wfUtils::formatLocalTime(get_option('date_format') . ' ' . get_option('time_format'), $nextUpdate));
						}
						else {
							$message .= ' ' . __('You may wait for the next automatic attempt or log into the parent site to manually update by clicking the "Manually Refresh Rules" button below the Rules list.', 'twodayssss');
						}
					}
		...
		if (!$warningAdded && $canDisplayMisconfiguredHowGetIPs && !wfUtils::truthyToBoolean(wfConfig::get('misconfiguredHowGetIPsChoice' . FIREWALL_VERSION)) && !(defined('FIREWALL_DISABLE_MISCONFIGURED_HOWGETIPS') && FIREWALL_DISABLE_MISCONFIGURED_HOWGETIPS)) {
			$warningAdded = true;
			if (wfUtils::isAdminPageMU()) {
		...
		$updatingNotifications = get_site_transient('FIREWALL_updating_notifications');
		...
		add_menu_page('twodayssss', "Wordfence{$dashboardExtra}", 'activate_plugins', 'twodayssss', 'wordfence::menu_dashboard', wfUtils::getBaseURL() . 'images/wordfence-logo.svg');
	}
	...
	public static function admin_menus_50() {
		add_submenu_page('twodayssss', 'Tools', 'Tools', 'activate_plugins', 'WordfenceTools', 'wordfence::menu_tools');
	...
	public static function admin_menus_70() {
		add_submenu_page('twodayssss', 'Help', 'Help', 'activate_plugins', 'WordfenceSupport', 'wordfence::menu_support');
	}
	
	public static function admin_menus_80() {
		if (wfCentral::isSupported()) {
			add_submenu_page(null, 'Wordfence Central', 'Wordfence Central', 'activate_plugins', 'WordfenceCentral', 'wordfence::menu_FIREWALL_central');
		}
	}
...
JQUERY;
	...
	private static function _menu_tools_livetraffic() {
		wp_enqueue_style('wordfence-jquery-ui-css', wfUtils::getBaseURL() . wfUtils::versionedAsset('css/jquery-ui.min.css'), array(), FIREWALL_VERSION);
		wp_enqueue_style('wordfence-jquery-ui-structure-css', wfUtils::getBaseURL() . wfUtils::versionedAsset('css/jquery-ui.structure.min.css'), array(), FIREWALL_VERSION);
		wp_enqueue_style('wordfence-jquery-ui-theme-css', wfUtils::getBaseURL() . wfUtils::versionedAsset('css/jquery-ui.theme.min.css'), array(), FIREWALL_VERSION);
		wp_enqueue_style('wordfence-jquery-ui-timepicker-css', wfUtils::getBaseURL() . wfUtils::versionedAsset('css/jquery-ui-timepicker-addon.css'), array(), FIREWALL_VERSION);
		
		wp_enqueue_script('wordfence-timepicker-js', wfUtils::getBaseURL() . wfUtils::versionedAsset('js/jquery-ui-timepicker-addon.js'), array('jquery', 'jquery-ui-datepicker', 'jquery-ui-slider'), FIREWALL_VERSION);
		wp_enqueue_script('wordfence-knockout-js', wfUtils::getBaseURL() . wfUtils::versionedAsset('js/knockout-3.3.0.js'), array(), FIREWALL_VERSION);
		wp_enqueue_script('wordfence-live-traffic-js', wfUtils::getBaseURL() . wfUtils::versionedAsset('js/admin.liveTraffic.js'), array('jquery', 'jquery-ui-tooltip'), FIREWALL_VERSION);
	...
	public static function menu_FIREWALL_central() {
		wfConfig::set('showWfCentralUI', 1);

		wp_enqueue_style('wordfence-select2-css');
		wp_enqueue_script('wordfence-select2-js');

		require(dirname(__FILE__) . '/menu_FIREWALL_central.php');
	}
...
HTML
	...
	public static function alert($subject, $alertMsg, $IP){
		...
		foreach ($emails as $email) {
			$uniqueContent = $content . "\n\n" . sprintf(__('No longer an administrator for this site? Click here to stop receiving security alerts: %s', 'twodayssss'), wfUtils::getSiteBaseURL() . '?_wfsf=removeAlertEmail&jwt=' . wfUtils::generateJWT(array('email' => $email)));
...
SQL
	...
	public static function ajax_email_summary_email_address_debug_callback() {
		...
		if (!wfUtils::isValidEmail($email)) {
			return array('err' => __('Invalid email address provided.', 'twodayssss'));
		}
	...
	public static function addDashboardWidget() {
		if (wfUtils::isAdmin() && (is_network_admin() || !is_multisite()) && wfConfig::get('email_summary_dashboard_widget_enabled')) {
			wp_enqueue_style('wordfence-activity-report-widget', wfUtils::getBaseURL() . wfUtils::versionedAsset('css/activity-report-widget.css'), '', FIREWALL_VERSION);
			...
			wp_add_dashboard_widget(
				'FIREWALL_activity_report_widget',
...
to your httpd.conf if using Apache, or find documentation on how to disable directory listing for your web server.",
	...
	public static function ajax_installAutoPrepend_callback() {
		...
		if ($serverConfiguration === null) {
			return array('errorMsg' => __('A valid server configuration was not provided.', 'twodayssss'));
		}
		...
		if (false === ($credentials = request_filesystem_credentials($ajaxURL, '', false, ABSPATH, array('version', 'locale', 'action', 'serverConfiguration', 'currentAutoPrepend'), $allow_relaxed_file_ownership))) {
			...
			$html = wfView::create('waf/waf-modal-wrapper', array(
				'title' => __('Filesystem Credentials Required', 'twodayssss'),
				'html' => $credentialsContent,
				'helpHTML' => sprintf(__('If you cannot complete the setup process, <a target="_blank" rel="noopener noreferrer" href="%s">click here for help</a>', 'twodayssss'), wfSupportController::esc_supportURL(wfSupportController::ITEM_FIREWALL_WAF_INSTALL_MANUALLY)),
				'footerHTML' => __('Once you have entered credentials, click Continue to complete the setup.', 'twodayssss'),
			))->render();
		...
		if (!WP_Filesystem($credentials, ABSPATH, $allow_relaxed_file_ownership) && $wp_filesystem->errors->get_error_code()) {
			...
			$html = wfView::create('waf/waf-modal-wrapper', array(
				'title' => __('Filesystem Permission Error', 'twodayssss'),
				'html' => $credentialsError,
				'helpHTML' => sprintf(__('If you cannot complete the setup process, <a target="_blank" rel="noopener noreferrer" href="%s">click here for help</a>', 'twodayssss'), wfSupportController::esc_supportURL(wfSupportController::ITEM_FIREWALL_WAF_INSTALL_MANUALLY)),
				'footerButtonTitle' => __('Cancel', 'twodayssss'),
			))->render();
		...
		try {
			...
			$verifyURL = add_query_arg(array('action' => 'FIREWALL_wafStatus', 'nonce' => $nonce), $ajaxURL);
			...
			if ($serverConfiguration == 'manual') {
				$html = wfView::create('waf/waf-modal-wrapper', array(
					'title' => __('Manual Installation Instructions', 'twodayssss'),
					'html' => wfView::create('waf/waf-install-manual')->render(),
					'footerButtonTitle' => __('Close', 'twodayssss'),
				))->render();
			}
			else {
				$html = wfView::create('waf/waf-modal-wrapper', array(
					'title' => __('Installation Successful', 'twodayssss'),
			...
			$html = wfView::create('waf/waf-modal-wrapper', array(
				'title' => __('Installation Failed', 'twodayssss'),
				'html' => $installError,
				'helpHTML' => sprintf(__('If you cannot complete the setup process, <a target="_blank" rel="noopener noreferrer" href="%s">click here for help</a>', 'twodayssss'), wfSupportController::esc_supportURL(wfSupportController::ITEM_FIREWALL_WAF_INSTALL_MANUALLY)),
				'footerButtonTitle' => __('Cancel', 'twodayssss'),
			))->render();
	...
	public static function ajax_uninstallAutoPrepend_callback() {
		...
		if (!isset($credentials)) {
			...
			if (false === ($credentials = request_filesystem_credentials($ajaxURL, '', false, ABSPATH, array('version', 'locale', 'action', 'serverConfiguration', 'iniModified'), $allow_relaxed_file_ownership))) {
				...
				$html = wfView::create('waf/waf-modal-wrapper', array(
					'title' => __('Filesystem Credentials Required', 'twodayssss'),
					'html' => $credentialsContent,
					'helpHTML' => sprintf(__('If you cannot complete the uninstall process, <a target="_blank" rel="noopener noreferrer" href="%s">click here for help</a>', 'twodayssss'), wfSupportController::esc_supportURL(wfSupportController::ITEM_FIREWALL_WAF_REMOVE_MANUALLY)),
					'footerHTML' => __('Once you have entered credentials, click Continue to complete uninstallation.', 'twodayssss'),
				))->render();
				return array('needsCredentials' => 1, 'html' => $html);
			}
			ob_end_clean();
		...
		try {
			if ((!isset($_POST['iniModified']) || (isset($_POST['iniModified']) && !$_POST['iniModified']))) { //Uses .user.ini but not yet modified
				...
				$waitingResponse = '<p>' . __('The <code>auto_prepend_file</code> setting has been successfully removed from <code>.htaccess</code> and <code>.user.ini</code>. Once this change takes effect, Extended Protection Mode will be disabled.', 'twodayssss') . '</p>';
				if ($hasPreviousAutoPrepend) {
					$waitingResponse .= '<p>' . __('Any previous value for <code>auto_prepend_file</code> will need to be re-enabled manually if still needed.', 'twodayssss') . '</p>';
				}
				
				$spinner = wfView::create('common/indeterminate-progress', array('size' => 32))->render();
				$waitingResponse .= '<ul class="wf-flex-horizontal"><li>' . $spinner . '</li><li class="wf-padding-add-left">' . sprintf(__('Waiting for it to take effect. This may take up to %s.', 'twodayssss'), $timeoutString) . '</li></ul>';
				
				$html = wfView::create('waf/waf-modal-wrapper', array(
					'title' => __('Waiting for Changes', 'twodayssss'),
				...
				if (WFWAF_AUTO_PREPEND && !WFWAF_SUBDIRECTORY_INSTALL && !WF_IS_WP_ENGINE) { //.user.ini modified, but the WAF is still enabled
					...
					$userIniError .= __('Extended Protection Mode has not been disabled. This may be because <code>auto_prepend_file</code> is configured somewhere else or the value is still cached by PHP.', 'twodayssss');
					if ($retryAttempted) {
						$userIniError .= ' <strong>' . __('Retrying Failed.', 'twodayssss') . '</strong>';
					}
					$userIniError .= ' <a href="#" class="wf-waf-uninstall-try-again">' . __('Try Again', 'twodayssss') . '</a>';
					$userIniError .= '</p>';
					$html = wfView::create('waf/waf-modal-wrapper', array(
						'title' => __('Unable to Uninstall', 'twodayssss'),
						'html' => $userIniError,
						'helpHTML' => sprintf(__('If you cannot complete the uninstall process, <a target="_blank" rel="noopener noreferrer" href="%s">click here for help</a>', 'twodayssss'), wfSupportController::esc_supportURL(wfSupportController::ITEM_FIREWALL_WAF_REMOVE_MANUALLY)),
						'footerButtonTitle' => __('Cancel', 'twodayssss'),
					))->render();
				...
				$html = wfView::create('waf/waf-modal-wrapper', array(
					'title' => __('Uninstallation Complete', 'twodayssss'),
			...
			$installError = "<p>" . $e->getMessage() . "</p>";
			$html = wfView::create('waf/waf-modal-wrapper', array(
				'title' => __('Uninstallation Failed', 'twodayssss'),
				'html' => $installError,
				'helpHTML' => sprintf(__('If you cannot complete the uninstall process, <a target="_blank" rel="noopener noreferrer" href="%s">click here for help</a>', 'twodayssss'), wfSupportController::esc_supportURL(wfSupportController::ITEM_FIREWALL_WAF_REMOVE_MANUALLY)),
				'footerButtonTitle' => __('Cancel', 'twodayssss'),
			))->render();
	...
	private static function scheduleSendAttackData($timeToSend = null) {
		...
		if (!wp_next_scheduled('FIREWALL_processAttackData')) {
			wp_schedule_single_event($timeToSend, 'FIREWALL_processAttackData');
		}
		if ($notMainSite) {
			restore_current_blog();
		}
...
ALERTMSG;
				...
				do_action('FIREWALL_security_event', 'increasedAttackRate', array(
	...
	public static function syncAttackData($exit = true) {
		...
		update_site_option('FIREWALL_syncingAttackData', 0);
		update_site_option('FIREWALL_syncAttackDataAttempts', 0);
		update_site_option('FIREWALL_lastSyncAttackData', time());
		if ($exit) {
			exit;
		}
	}
	...
	public static function addSyncAttackDataAjax() {
		$URL = home_url('/?FIREWALL_syncAttackData=' . microtime(true));
	...
	public static function wafAutoPrependNotice() {
		...
		Application Firewall:', 'twodayssss') . ' &nbsp;<a class="wf-btn wf-btn-default wf-btn-sm" href="' . esc_url($url) . '">' . __('Click here to configure', 'twodayssss') . '</a>
		<a class="wf-btn wf-btn-default wf-btn-sm wf-dismiss-link" href="#"  onclick="wordfenceExt.setOption(\'dismissAutoPrependNotice\', 1); jQuery(\'#wf-extended-protection-notice\').fadeOut(); return false;">' . __('Dismiss', 'twodayssss') . '</a>
		<br>
		<em style="font-size: 85%;">' . sprintf(__('If you cannot complete the setup process, <a target="_blank" rel="noopener noreferrer" href="%s">click here for help</a>.', 'twodayssss'), wfSupportController::esc_supportURL(wfSupportController::ITEM_FIREWALL_WAF_INSTALL_MANUALLY)) . '</em>
		</div>';
	}
...
?>',
			var_export(TF_PATH . 'waf/bootstrap.php', true),
	...
	public static function ajax_wfcentral_step1_callback() {
		...
		if (!$authGrant) {
			return array(
				'err'      => 1,
				'errorMsg' => __("Auth grant is invalid.", 'twodayssss'),
			);
		}
	...
	public static function ajax_wfcentral_step2_callback() {
		...
		require_once(TF_PATH . '/crypto/vendor/paragonie/sodium_compat/autoload-fast.php');

		$accessToken = wfConfig::get('wordfenceCentralAccessToken');
		if (!$accessToken) {
			return array(
				'err'      => 1,
				'errorMsg' => __("Access token not found.", 'twodayssss'),
			);
		}
	...
	public static function ajax_wfcentral_step3_callback() {
		...
		try {
			...
			if (!is_array($body) || !isset($body['data']['attributes'])) {
				return array(
					'error' => 1,
					'errorMsg' => __('Invalid response from Wordfence Central.', 'twodayssss'),
				);
			}
	...
	public static function ajax_wfcentral_step5_callback() {
		...
		if (!$wfCentralAuthGrant) {
			return array(
				'error' => 1,
				'errorMsg' => __('Auth grant not found.', 'twodayssss'),
			);
		}
		...
		try {
			...
			return array(
				...
				'redirect-url' => sprintf(FIREWALL_CENTRAL_URL_SEC . '/sites/%s?access-token=%s',
