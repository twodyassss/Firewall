...
class wfCrawl {
	...
	public static function verifyCrawlerPTR($hostPattern, $IP){
		...
		$status = $db->querySingle("select status from $table where IP=%s and patternSig=UNHEX(MD5('%s')) and lastUpdate > unix_timestamp() - %d", $IPn, $hostPattern, FIREWALL_CRAWLER_VERIFY_CACHE_TIME);
		if($status){
			if($status == 'verified'){
				return true;
			} else {
...
'@^Google$@'
	...
	public static function verifyGooglebotViaNOC1($ip = null) {
		...
		$status = $db->querySingle("select status from $table
				...
				$IPn,
				$patternSig,
				FIREWALL_CRAWLER_VERIFY_CACHE_TIME);
		if ($status === 'verified') {
