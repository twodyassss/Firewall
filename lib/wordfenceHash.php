						...
						if (strpos($realFile, $path) === 0) {
							if ($this->isPreviousCoreFile($shac)) {
								$added = $this->engine->addIssue(
									...
									sprintf(__('Old WordPress core file not removed during update: %s', 'twodayssss'), $file),
									__('This file is in a WordPress core location but is from an older version of WordPress and not used with your current version. Hosting or permissions issues can cause these files to get left behind when WordPress is updated and they should be removed if possible.', 'twodayssss'),
									array(
										'file' => $file,
										'cType' => 'core',
										'canDiff' => false,
								...
								$this->engine->addPendingIssue(
									...
									'coreUnknown' . $file,
									'coreUnknown' . $file . $md5,
									sprintf(__('Unknown file in WordPress core: %s', 'twodayssss'), $file),
									__('This file is in a WordPress core location but is not distributed with this version of WordPress. This scan often includes files left over from a previous WordPress version, but it may also find files added by another plugin, files added by your host, or malicious files added by an attacker.', 'twodayssss'),
									array(
										'file' => $file,
										'cType' => 'core',
										'canDiff' => false,
								...
								$added = $this->engine->addIssue(
									...
									'coreUnknown' . $file . $md5,
									sprintf(__('Unknown file in WordPress core: %s', 'twodayssss'), $file),
									sprintf(__('This file is in a WordPress core location but is not distributed with this version of WordPress. This scan often includes files left over from a previous WordPress version, but it may also find files added by another plugin, files added by your host, or malicious files added by an attacker. <a href="%s" target="_blank" rel="noopener noreferrer">Learn More</a>', 'twodayssss'), wfSupportController::esc_supportURL(wfSupportController::ITEM_SCAN_RESULT_UNKNOWN_FILE_CORE)),
									array(
										'file' => $file,
										'cType' => 'core',
										'canDiff' => false,
	...
	private function _processPendingIssues() {
		...
		foreach ($this->coalescingIssues as $c) {
			...
			$added = $this->engine->addIssue(
				...
				$i['ignoreP'],
				$i['ignoreC'],
				$i['shortMsg'] . ($count > 1 ? ' ' . sprintf(__('(+ %d more)', 'twodayssss'), $count - 1) : ''),
				$i['longMsg'] . ($count > 1 ? ' ' . ($count > 2 ? sprintf(__('%d more similar files were found.', 'twodayssss'), $count - 1) : __('1 more similar file was found.', 'twodayssss')) : '') . (isset($i['data']['learnMore']) ? ' ' . sprintf(__('<a href="%s" target="_blank" rel="noopener noreferrer">Learn More</a>', 'twodayssss'), esc_attr($i['data']['learnMore'])) : ''),
				$i['data'],
				true //Prevent ignoreP and ignoreC from being hashed again
			);
			if ($added == wfIssues::ISSUE_ADDED || $added == wfIssues::ISSUE_UPDATED) { $this->haveIssues[$haveIssuesType] = wfIssues::STATUS_PROBLEM; }
