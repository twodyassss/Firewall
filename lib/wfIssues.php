...
class wfIssues {
	...
	public static function hasScanFailed() {
		...
		if ($lastStatusUpdate !== false && wfScanner::shared()->isRunning()) {
			$threshold = FIREWALL_SCAN_FAILURE_THRESHOLD;
		...
		if ($scanStartAttempt && time() - $scanStartAttempt > FIREWALL_SCAN_START_FAILURE_THRESHOLD) {
	...
	public function emailNewIssues($timeLimitReached = false, $scanController = false){
		...
		foreach ($emails as $email) {
			$uniqueContent = str_replace('<!-- ##UNSUBSCRIBE## -->', sprintf(__('No longer an administrator for this site? <a href="%s" target="_blank">Click here</a> to stop receiving security alerts.', 'twodayssss'), wfUtils::getSiteBaseURL() . '?_wfsf=removeAlertEmail&jwt=' . wfUtils::generateJWT(array('email' => $email))), $content);
