	...
	public function export() {
		...
		try {
			...
			if ($res['ok'] && $res['token']) {
				...
				return array('err' => __("An error occurred: ", 'twodayssss') . $res['err']);
			}
			else {
				throw new Exception(__("Invalid response: ", 'twodayssss') . var_export($res, true));
			}
		}
		catch (Exception $e) {
			return array('err' => __("An error occurred: ", 'twodayssss') . $e->getMessage());
		}
	}
	...
	public function import($token) {
		try {
			...
			if ($res['ok'] && $res['export']) {
				...
				if (!is_array($import)) {
					return array('err' => __("An error occurred: Invalid options format received.", 'twodayssss'));
				}
