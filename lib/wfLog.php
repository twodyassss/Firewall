...
class wfLog {
	...
	public function tagRequestForBlock($reason, $wfsn = false) {
		if ($this->currentRequest !== null) {
			...
			$this->currentRequest->action = 'blocked:' . ($wfsn ? 'wfsn' : 'twodayssss');
	...
	public function firewallBadIPs() {
		...
		foreach ($countryBlocks as $b) {
			...
			if ($match === wfBlock::MATCH_COUNTRY_REDIR_BYPASS) {
				...
				$this->initLogRequest();
				$this->getCurrentRequest()->actionDescription = __('redirected to bypass URL', 'twodayssss');
				...
				$this->initLogRequest();
				$this->getCurrentRequest()->actionDescription = sprintf(__('blocked access via country blocking and redirected to URL (%s)', 'twodayssss'), wfConfig::get('cbl_redirURL'));
				...
				$b->recordBlock();
				$this->currentRequest->actionDescription = __('blocked access via country blocking', 'twodayssss');
				wfConfig::inc('totalCountryBlocked');
				wfActivityReport::logBlockedIP($IP, null, 'country');
				$this->do503(3600, __('Access from your area has been temporarily limited for security reasons', 'twodayssss'));
			}
		}
		...
		if ($ipBlock !== false) {
			...
			if ($ipBlock->type == wfBlock::TYPE_IP_MANUAL || $ipBlock->type == wfBlock::TYPE_IP_AUTOMATIC_PERMANENT) {
				$reason = __('Manual block by administrator', 'twodayssss');
			}
	...
	private function takeBlockingAction($configVar, $reason) {
		if ($this->googleSafetyCheckOK()) {
			...
			if ($action == 'block') { //Rate limited - block temporarily
				...
				do_action('FIREWALL_security_event', 'block', array(
					'ip' => $IP,
					'reason' => $reason,
					'duration' => $secsToGo,
				), $alertCallback);
				wordfence::status(2, 'info', sprintf(__('Blocking IP %s. %s', 'twodayssss'), $IP, $reason));
			}
				...
				do_action('FIREWALL_security_event', 'throttle', array(
					'ip' => $IP,
					'reason' => $reason,
					'duration' => $secsToGo,
				));
				wordfence::status(2, 'info', sprintf(__('Throttling IP %s. %s', 'twodayssss'), $IP, $reason));
