<?php
class wfConfig {
	const TABLE_EXISTS_OPTION = 'FIREWALL_installed';
	...
	public static function loadAllOptions() {
		...
		$options = wp_cache_get('alloptions', 'twodayssss');
		if (!$options) {
			$table = self::table();
			...
			wp_cache_add_non_persistent_groups('twodayssss');
			wp_cache_add('alloptions', $options, 'twodayssss');
		}
	...
	private static function updateCachedOption($name, $val) {
		...
		$options[$name] = $val;
		wp_cache_set('alloptions', $options, 'twodayssss');
	}
	...
	private static function ser_chunked_key($key) {
		return 'FIREWALL_chunked_' . $key . '_';
	}
	...
	public static function liveTrafficEnabled(&$overriden = null){
		...
		if (FIREWALL_DISABLE_LIVE_TRAFFIC || WF_IS_WP_ENGINE) {
			$enabled = false;
			if ($overriden !== null) {
				$overriden = true;
			}
	...
	public static function enableAutoUpdate(){
		...
		if (is_main_site()) {
			wp_schedule_event(time(), 'daily', 'FIREWALL_daily_autoUpdate');
		}
	}
	...
	public static function autoUpdate(){
		...
		if ($update_plugins && is_array($update_plugins->response) && isset($update_plugins->response[TF_BASENAME])) {
			$status = $update_plugins->response[TF_BASENAME];
			if (is_object($status) && property_exists($status, 'new_version')) {
				$runUpdate = (version_compare($status->new_version, FIREWALL_VERSION) > 0);
			}
		}
		...
		if ($runUpdate) {
			try {
				...
				$response = $api->call('should_auto_update', array(), array('currentVersion' => FIREWALL_VERSION));
		...
		try {
			...
			*/
			...
			$upret = $upgrader->upgrade(TF_BASENAME);
			if($upret){
				$cont = file_get_contents(TF_FCPATH);
				...
				do_action('FIREWALL_security_event', 'autoUpdate', array(
					'version' => $version,
					'ip' => wfUtils::getIP(),
				), $alertCallback);
			...
			switch ($key) {
				...
				case 'learningModeGracePeriod':
				{
					...
					if ($wafStatus == wfFirewall::FIREWALL_MODE_LEARNING && $gracePeriodEnd <= time()) {
						$errors[] = array('option' => $key, 'error' => __('The grace period end time must be in the future.', 'twodayssss'));
					}
				...
				case 'wafStatus':
				{
					if ($value != wfFirewall::FIREWALL_MODE_ENABLED && $value != wfFirewall::FIREWALL_MODE_LEARNING && $value != wfFirewall::FIREWALL_MODE_DISABLED) {
						$errors[] = array('option' => $key, 'error' => __('Unknown firewall mode.', 'twodayssss'));
					}
				...
				case 'alertEmails':
				{
					...
					if (count($badEmails) > 0) {
						$errors[] = array('option' => $key, 'error' => __('The following emails are invalid: ', 'twodayssss') . esc_html(implode(', ', $badEmails), array()));
					}
				...
				case 'scan_include_extra':
				{
					...
					foreach ($dirtyRegexes as $regex) {
						if (@preg_match("/$regex/", "") === false) {
							$errors[] = array('option' => $key, 'error' => sprintf(__('"%s" is not a valid regular expression.', 'twodayssss'), esc_html($regex)));
						}
					}
					$checked = true;
					break;
				...
				case 'whitelisted':
				{
					...
					if (count($badWhiteIPs) > 0) {
						$errors[] = array('option' => $key, 'error' => __('Please make sure you separate your IP addresses with commas. The following whitelisted IP addresses are invalid: ', 'twodayssss') . esc_html(implode(', ', $badWhiteIPs), array()));
					}
				...
				case 'liveTraf_ignoreUsers':
				{
					...
					if (count($invalidUsers) > 0) {
						$errors[] = array('option' => $key, 'error' => __('The following users you selected to ignore in live traffic reports are not valid on this system: ', 'twodayssss') . esc_html(implode(', ', $invalidUsers), array()));
					}
				...
				case 'liveTraf_ignoreIPs':
				{
					...
					if (count($invalidIPs) > 0) {
						$errors[] = array('option' => $key, 'error' => __('The following IPs you selected to ignore in live traffic reports are not valid: ', 'twodayssss') . esc_html(implode(', ', $invalidIPs), array()));
					}
				...
				case 'howGetIPs_trusted_proxies':
				{
					...
					if (count($invalidIPs) > 0) {
						$errors[] = array('option' => $key, 'error' => __('The following IPs/ranges you selected to trust as proxies are not valid: ', 'twodayssss') . esc_html(implode(', ', $invalidIPs), array()));
					}
				...
				case 'apiKey':
				{
					$value = trim($value);
					if (empty($value)) {
						$errors[] = array('option' => $key, 'error' => __('An empty license key was entered.', 'twodayssss'));
					}
					else if ($value && !preg_match('/^[a-fA-F0-9]+$/', $value)) {
						$errors[] = array('option' => $key, 'error' => __('The license key entered is not in a valid format. It must contain only numbers and the letters A-F.', 'twodayssss'));
					}
			...
			switch ($key) {
				...
				case 'wafStatus':
				{
					...
					if ($value == wfFirewall::FIREWALL_MODE_DISABLED) {
						...
						do_action('FIREWALL_security_event', 'wafDeactivated', array(
							'username' => $username,
							'ip' => wfUtils::getIP(),
						), $alertCallback);
					}
		...
		if ($apiKey !== false) {
			...
			if (empty($apiKey)) { //Empty, try getting a free key
				...
				try {
					...
					throw new wfConfigException(__('Your options have been saved, but you left your license key blank, so we tried to get you a free license key from the Wordfence servers. There was a problem fetching the free key: ', 'twodayssss') . wp_kses($e->getMessage(), array()));
				}
			}
				...
				try {
					...
					throw new wfConfigException(__('Your options have been saved. However we noticed you changed your license key, and we tried to verify it with the Wordfence servers but received an error: ', 'twodayssss') . wp_kses($e->getMessage(), array()));
				}
			}
			...
			if ($ping) {
				...
				try {
					...
					throw new wfConfigException(__('Your options have been saved. However we tried to verify your license key with the Wordfence servers and received an error: ', 'twodayssss') . wp_kses($e->getMessage(), array()));
				}
			}
		}
