...
class wfScanEngine {
	...
	public function submitMetrics() {
		if (wfConfig::get('other_WFNet', true)) {
			...
			if (isset($this->metrics['malwareSignature'])) {
				...
				$overage = $extra_count - FIREWALL_SCAN_ISSUES_MAX_REPORT;
				if ($overage > 0) {
					foreach ($this->metrics['malwareSignature'] as $rule => $payloads) {
				...
				//Trim single matches
				if ($count > FIREWALL_SCAN_ISSUES_MAX_REPORT) {
					$sliced = array_slice($this->metrics['malwareSignature'], 0, FIREWALL_SCAN_ISSUES_MAX_REPORT, true);
	...
	private function scan_checkHowGetIPs_main() {
		if (!defined('FIREWALL_CHECKHOWGETIPS_TIMEOUT')) { define('FIREWALL_CHECKHOWGETIPS_TIMEOUT', 30); }
		
		$haveIssues = wfIssues::STATUS_SECURE;
		$existing = wfConfig::get('howGetIPs', '');
		$recommendation = wfConfig::get('detectProxyRecommendation', '');
		while (empty($recommendation) && (time() - $this->checkHowGetIPsRequestTime) < FIREWALL_CHECKHOWGETIPS_TIMEOUT) {
			sleep(1);
			$this->forkIfNeeded();
			$recommendation = wfConfig::get('detectProxyRecommendation', '');
		}
...
//			wfCommonBackupFileTest::createFromRootPath('.htaccess'),
	...
	private function scan_checkSkippedFiles() {
		...
		if (!empty($paths['skipped'])) {
			...
			foreach ($paths['skipped'] as $index => $path) {
				if ($index >= 10) {
					$skippedList .= sprintf(__(', and %d more.', 'twodayssss'), count($paths['skipped']) - 10);
					break;
				}
			...
			$added = $this->addIssue(
				...
				$key,
				$key,
				sprintf($c == 1 ? __('%d path was skipped for the malware scan due to scan settings', 'twodayssss') : __('%d paths were skipped for the malware scan due to scan settings', 'twodayssss'), $c),
				sprintf($c == 1 ? __('The option "Scan files outside your WordPress installation" is off by default, which means %d path and its file(s) will not be scanned for malware or unauthorized changes. To continue skipping this path, you may ignore this issue. Or to start scanning it, enable the option and subsequent scans will include it. Some paths may not be necessary to scan, so this is optional. <a href="%s" target="_blank" rel="noopener noreferrer">Learn More</a><br><br>The path skipped is %s', 'twodayssss') : __('The option "Scan files outside your WordPress installation" is off by default, which means %d paths and their file(s) will not be scanned for malware or unauthorized changes. To continue skipping these paths, you may ignore this issue. Or to start scanning them, enable the option and subsequent scans will include them. Some paths may not be necessary to scan, so this is optional. <a href="%s" target="_blank" rel="noopener noreferrer">Learn More</a><br><br>The paths skipped are %s', 'twodayssss'), $c, wfSupportController::esc_supportURL(wfSupportController::ITEM_SCAN_RESULT_SKIPPED_PATHS), $skippedList),
				array()
			);
	...
	private function scan_passwds_init(){
		...
		$dbh = $wpdb->dbh;
		$useMySQLi = (is_object($dbh) && $wpdb->use_mysqli && wfConfig::get('allowMySQLi', true) && FIREWALL_ALLOW_DIRECT_MYSQLI);
	...
	*/
	private function scan_diskSpace() {
		$this->statusIDX['diskSpace'] = wfIssues::statusStart(__('Scanning to check available disk space', 'twodayssss'));
		$this->scanController->startStage(wfScanner::STAGE_SERVER_STATE);
		wfUtils::errorsOff();
		...
		wfUtils::errorsOn();
		if (!$total || !$free) {
			$this->status(2, 'info', __('Unable to access available disk space information', 'twodayssss'));
		...
		$this->status(2, 'info', sprintf(__('Total disk space: %s -- Free disk space: %s', 'twodayssss'), wfUtils::formatBytes($total), wfUtils::formatBytes($free)));
		$freeMegs = round($free / 1024 / 1024, 2);
		$this->status(2, 'info', sprintf(__('The disk has %s MB available', 'twodayssss'), $freeMegs));
		if ($freeMegs < 5) {
			$level = wfIssues::SEVERITY_CRITICAL;
		}
		else if ($freeMegs < 20) {
		...
		$added = $this->addIssue('diskSpace', 
			...
			'diskSpace', 
			'diskSpace' . $level, 
			sprintf(__('You have %s disk space remaining', 'twodayssss'), wfUtils::formatBytes($free)), 
			sprintf(__('You only have %s of your disk space remaining. Please free up disk space or your website may stop serving requests.', 'twodayssss'), wfUtils::formatBytes($free)), 
	...
	private function scan_wafStatus() {
		$this->statusIDX['wafStatus'] = wfIssues::statusStart(__('Checking Web Application Firewall status', 'twodayssss'));
		$this->scanController->startStage(wfScanner::STAGE_SERVER_STATE);
	...
	public function scan_geoipSupport() {
		...
		if (version_compare(phpversion(), '5.4') < 0 && wfConfig::get('isPaid') && wfBlock::hasCountryBlock()) {
			$shortMsg = __('PHP Update Needed for Country Blocking', 'twodayssss');
			$longMsg = sprintf(__('The GeoIP database that is required for country blocking has been updated to a new format. This new format requires sites to run PHP 5.4 or newer, and this site is on PHP %s. To ensure country blocking continues functioning, please update PHP.', 'twodayssss'), wfUtils::cleanPHPVersion());
	...
	public static function startScan($isFork = false, $scanMode = false){
		...
		$testURL = admin_url('admin-ajax.php?action=FIREWALL_testAjax');
	...
	protected static function _baseStartURL($isFork, $scanMode, $cronKey) {
		...
		$url .= '?action=FIREWALL_doScan&isFork=' . ($isFork ? '1' : '0') . '&scanMode=' . urlencode($scanMode) . '&cronKey=' . urlencode($cronKey);
		return $url;
	}
	...
	protected static function _remoteStartURL($isFork, $scanMode, $cronKey) {
		$url = self::_baseStartURL($isFork, $scanMode, $cronKey);
		$url = preg_replace('/^https?:\/\//i', (wfAPI::SSLEnabled() ? FIREWALL_API_URL_SEC : FIREWALL_API_URL_NONSEC) . 'scanp/', $url);
	...
	public static function getMaxExecutionTime($staySilent = false) {
		...
		if (is_numeric($config) && $config >= FIREWALL_SCAN_MIN_EXECUTION_TIME) {
		...
		if (is_numeric($ini) && $ini >= FIREWALL_SCAN_MIN_EXECUTION_TIME) {
			if ($ini > FIREWALL_SCAN_MAX_INI_EXECUTION_TIME) {
				if (!$staySilent) { wordfence::status(4, 'info', "ini value of {$ini} is higher than value for FIREWALL_SCAN_MAX_INI_EXECUTION_TIME (" . FIREWALL_SCAN_MAX_INI_EXECUTION_TIME . "), reducing"); }
				$ini = FIREWALL_SCAN_MAX_INI_EXECUTION_TIME;
			}
