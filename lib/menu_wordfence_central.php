...
$stepContent = array(
	1 => __('Testing initial communication with Wordfence Central.', 'twodayssss'),
	2 => __('Passing public key to Wordfence Central.', 'twodayssss'),
	3 => __('Testing public key authentication with Wordfence Central.', 'twodayssss'),
	4 => __('Testing that Wordfence Central is able to communicate with this site.', 'twodayssss'),
	5 => __('Retrieving access token using authorization grant.', 'twodayssss'),
	6 => __('Redirecting back to Wordfence Central.', 'twodayssss'),
);
		...
		<div class="wf-row">
			<div class="wf-col-xs-12">
				...
				echo wfView::create('common/section-title', array(
					'title'    => __('Wordfence Central', 'twodayssss'),
					'showIcon' => true,
				))->render();
				?>
			</div>
						...
						<div class="wf-flex-row-1 wf-block wf-active">
							<div class="wf-central-dashboard">
								...
								<div class="wf-central-dashboard-copy">
									<p><strong><?php _e('Wordfence Central', 'twodayssss') ?></strong></p>
									<p><?php _e('Wordfence Central allows you to manage Wordfence on multiple sites from one location. It makes security monitoring and configuring Wordfence easier.', 'twodayssss') ?></p>
									<p class="wf-right-lg"><a href="https://www.wordfence.com/central" target="_blank" rel="noopener noreferrer"><strong><?php _e('Visit Wordfence Central', 'twodayssss') ?></strong></a></p>
								</div>
							</div>
						</div>
						<div class="wf-flex-row-1 wf-block wf-active">
							<p><strong><?php _e('Wordfence Central Status', 'twodayssss') ?></strong></p>
							<p><?php printf(__('Activated - connected by %s on %s', 'twodayssss'), esc_html(wfConfig::get('wordfenceCentralConnectEmail')), date_i18n('F j, Y', (int) wfConfig::get('wordfenceCentralConnectTime'))) ?></p>
							<p class="wf-right-lg"><a href="<?php echo esc_url($wordfenceURL); ?>"><strong><?php _e('Disconnect This Site', 'twodayssss') ?></strong></a></p>
						</div>
					</div>
				</div>
			...
			<?php elseif ($partialConnection): ?>
				<div class="wf-center-lg">
					<p><?php _e('It looks like you\'ve tried to connect this site to Wordfence Central, but the installation did not finish.', 'twodayssss') ?></p>
					<p>
						...
						<a href="<?php echo esc_url($wordfenceURL); ?>" class="wf-btn wf-btn-warning"><?php _e('Disconnect Site', 'twodayssss') ?></a>
					</p>
				</div>
			<?php else: ?>
				<div class="wf-center-lg">
					<p><?php _e('Wordfence Central allows you to manage Wordfence on multiple sites from one location. It makes security monitoring and configuring Wordfence easier.', 'twodayssss') ?></p>
					<p><?php _e('To connect your site your site to Wordfence Central, use the link below:', 'twodayssss') ?></p>
					<p class="wf-center">
						<a href="<?php echo FIREWALL_CENTRAL_URL_SEC ?>?newsite=<?php echo esc_attr(home_url()) ?>" class="wf-btn wf-btn-primary"><?php _e('Connect Site', 'twodayssss') ?></a>
					</p>
				</div>
			<?php endif ?>
		</div>
...
<script>
	(function($) {
		...
		WFCentralInstaller.step1 = function() {
			wfCentralStepAjax('step1', 'FIREWALL_wfcentral_step1', {
		...
		WFCentralInstaller.step2 = function() {
			wfCentralStepAjax('step2', 'FIREWALL_wfcentral_step2', {}, function(response) {
		...
		WFCentralInstaller.step3 = function() {
			wfCentralStepAjax('step3', 'FIREWALL_wfcentral_step3', {}, function(response) {
		...
		WFCentralInstaller.step4 = function() {
			wfCentralStepAjax('step4', 'FIREWALL_wfcentral_step4', {}, function(response) {
				if (response && response.success) {
		...
		WFCentralInstaller.step5 = function() {
			wfCentralStepAjax('step5', 'FIREWALL_wfcentral_step5', {
				'auth-grant': authGrant
			}, function(response) {
		...
		WFCentralInstaller.step6 = function(response) {
			wfCentralStepAjax('step6', 'FIREWALL_wfcentral_step6', {}, function(response) {
