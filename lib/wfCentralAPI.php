...
class wfCentralAPIRequest {
	...
	public function execute() {
		...
		$response = $http->request(FIREWALL_CENTRAL_API_URL_SEC . $this->getEndpoint(), $args);
...
class wfCentralAPIResponse {
	...
	public function returnErrorArray() {
		return array(
			'err'      => 1,
			'errorMsg' => sprintf(__('HTTP %d received from Wordfence Central: %s', 'twodayssss'),
...
class wfCentralAuthenticatedAPIRequest extends wfCentralAPIRequest {
	...
	public function getToken() {
		...
		if (empty($token)) {
			if (isset($e)) {
				...
				throw new wfCentralAPIException(__('Unable to authenticate with Wordfence Central.', 'twodayssss'));
			}
		}
	...
	public function fetchToken() {
		...
		if (!$siteID) {
			throw new wfCentralAPIException(__('Wordfence Central site ID has not been created yet.', 'twodayssss'));
		}
		$secretKey = wfConfig::get('wordfenceCentralSecretKey');
		if (!$secretKey) {
			throw new wfCentralAPIException(__('Wordfence Central secret key has not been created yet.', 'twodayssss'));
		}
		...
		if (!is_array($body) || !isset($body['nonce'])) {
			throw new wfCentralAPIException(__('Invalid response received from Wordfence Central when fetching nonce.', 'twodayssss'));
		}
		$nonce = $body['nonce'];
		...
		if (!is_array($body)) {
			throw new wfCentralAPIException(__('Invalid response received from Wordfence Central when fetching token.', 'twodayssss'));
		}
