...
class wfRESTAuthenticationController {
	...
	public function authenticate($request) {
		...
		if (!$siteID) {
			return new WP_Error('rest_forbidden_context',
				__('Site is not connected to Wordfence Central.', 'twodayssss'),
		...
		if (count($dataChunks) !== 2) {
			return new WP_Error('rest_forbidden_context',
				__('Data is invalid.', 'twodayssss'),
				array('status' => rest_authorization_required_code()));
		}
		if (!preg_match('/[0-9a-f]{64}/i', $dataChunks[0])) {
			return new WP_Error('rest_forbidden_context',
				__('Nonce format is invalid.', 'twodayssss'),
				array('status' => rest_authorization_required_code()));
		}
		if (!preg_match('/[0-9a-f\-]{36}/i', $dataChunks[1])) {
			return new WP_Error('rest_forbidden_context',
				__('Site ID is invalid.', 'twodayssss'),
				array('status' => rest_authorization_required_code()));
		}
		...
		if (!$verfiedNonce) {
			return new WP_Error('rest_forbidden_context',
				__('Nonce is invalid.', 'twodayssss'),
				array('status' => rest_authorization_required_code()));
		}
		...
		if (!ParagonIE_Sodium_Compat::crypto_sign_verify_detached($signature, $data, wfConfig::get('wordfenceCentralPK'))) {
			return new WP_Error('rest_forbidden_context',
				__('Signature is invalid.', 'twodayssss'),
				array('status' => rest_authorization_required_code()));
		}
	...
	public function authenticatePremium($request) {
		...
		if (!is_email($dataChunks[1])) {
			return new WP_Error('rest_forbidden_context',
				__('Email address is invalid.', 'twodayssss'),
				array('status' => rest_authorization_required_code()));
		}
		...
		$signature = pack('H*', $signature);
		if (!ParagonIE_Sodium_Compat::crypto_sign_verify_detached($signature, $data, FIREWALL_CENTRAL_PUBLIC_KEY)) {
			return new WP_Error('rest_forbidden_context',
				__('Signature is invalid.', 'twodayssss'),
				array('status' => rest_authorization_required_code()));
		}
		...
		return new WP_Error('rest_forbidden_context',
			__('Admin user with this email address not found.', 'twodayssss'),
			array('status' => rest_authorization_required_code()));
	}
