...
class wfRESTScanController extends wfRESTBaseController {
	...
	public function registerRoutes() {
		register_rest_route('wordfence/v1', '/scan/issues', array(
			...
			'group'               => array(
				'description' => __('Scan result group or all results.', 'twodayssss'),
				'type'        => 'string',
				'required'    => false,
			),
			'offset'              => array(
				'description' => __('Offset of scan results to return.', 'twodayssss'),
				'type'        => 'int',
				'required'    => false,
			),
			'limit'               => array(
				'description' => __('Number of scan results to return.', 'twodayssss'),
				'type'        => 'int',
				'required'    => false,
			),
		));
	...
	public function startScan($request) {
		wordfence::status(1, 'info', sprintf(__('Wordfence scan starting at %s from Wordfence Central', 'twodayssss'),
	...
	public function stopScan($request) {
		wordfence::status(1, 'info', __('Scan stop request received from Wordfence Central.', 'twodayssss'));
		wordfence::status(10, 'info', __('SUM_KILLED:A request was received to stop the previous scan from Wordfence Central.', 'twodayssss'));
