...
class wfRESTConfigController extends wfRESTBaseController {
	...
	public function registerRoutes() {
		register_rest_route('wordfence/v1', '/config', array(
			...
			'fields'              => array(
				'description' => __('Specific config options to return.', 'twodayssss'),
				'type'        => 'array',
				'required'    => false,
			),
		));
		register_rest_route('wordfence/v1', '/config', array(
			...
			'permission_callback' => array($this, 'verifyToken'),
			'fields'              => array(
				'description' => __('Specific config options to set.', 'twodayssss'),
				'type'        => 'array',
				'required'    => true,
			),
		));
	...
	public function setConfig($request) {
			...
			if ($loginSecurityConfig) {
				...
				if ($errors !== true) {
					if (count($errors) == 1) {
						return new WP_Error('rest_set_config_error',
							sprintf(__('An error occurred while saving the configuration: %s', 'twodayssss'), $errors[0]['error']),
							array('status' => 422));
						...
						return new WP_Error('rest_set_config_error',
							sprintf(__('Errors occurred while saving the configuration: %s', 'twodayssss'), implode(', ', $compoundMessage)),
							array('status' => 422));
					}

					return new WP_Error('rest_set_config_error',
						__('Errors occurred while saving the configuration.', 'twodayssss'),
						array('status' => 422));
				}
				...
				try {
					...
					return new WP_Error('rest_save_config_error',
						sprintf(__('A server error occurred while saving the configuration: %s', 'twodayssss'), $e->getMessage()),
						array('status' => 500));
				}
			}
		...
		return new WP_Error('rest_save_config_error',
			__("Validation error: 'fields' parameter is empty or not an array.", 'twodayssss'),
			array('status' => 422));
