...
switch ($action) {
	case 'restoreFile':
		$callback = array('twodayssss', 'fsActionRestoreFileCallback');
		break;
	case 'deleteFile':
		$callback = array('twodayssss', 'fsActionDeleteFileCallback');
		break;
}
?>
	...
	<div class="wf-row">
		<div class="wf-col-xs-12">
			...
			echo wfView::create('options/block-controls', array(
				'backLink' => $scanURL,
				'backLabel' => __('Back to Scan', 'twodayssss'),
			...
			<div class="<?php echo wfStyle::contentClasses(); ?>">
				<div id="wf-scan-permissions-prompt" class="wf-fixed-tab-content">
					...
					echo wfView::create('common/section-title', array(
						'title' => __('File System Credentials Required', 'twodayssss'),
					))->render();
					?>
					<div class="wf-row">
						<div class="wf-col-xs-12">
							<div class="wf-block wf-active">
								<div class="wf-block-content wf-padding-add-top wf-padding-add-bottom">
									...
									if (isset($_GET['nonce']) && wp_verify_nonce($_GET['nonce'], 'wp-ajax')) {
										...
										echo '<p>' . sprintf(__('Security token has expired. Click <a href="%s">here</a> to return to the scan page.', 'twodayssss'), esc_url($scanURL)) . '</p>';
									}
									?>
								</div>
							</div>
