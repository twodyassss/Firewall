			...
			<div class="wf-dashboard-item-inner">
				<div class="wf-dashboard-item-content">
					<div class="wf-dashboard-item-title">
						<strong><?php _e('Firewall Summary:', 'twodayssss'); ?> </strong><?php printf(__('Attacks Blocked for %s', 'twodayssss'), esc_html(preg_replace('/^[^:]+:\/\//', '', network_site_url()))); ?>
					</div>
			...
			<div class="wf-dashboard-item-extra">
				...
				<ul class="wf-dashboard-item-list">
					<li class="wf-flex-vertical wf-flex-full-width">
						...
						if (!$hasSome):
						?>
							<div class="wf-dashboard-item-list-text"><p><em><?php _e('No blocks have been recorded.', 'twodayssss'); ?></em></p></div>
						<?php else: ?>
							<table class="wf-blocks-summary">
								<thead>
								<tr>
									<th><?php _e('<span class="wf-hidden-xs">Block </span>Type', 'twodayssss'); ?></th>
								<?php
								...
								<?php endforeach; ?>
									<th width="25%"><?php _e('Total', 'twodayssss'); ?></th>
								</tr>
								</thead>
								<tbody>
								<?php
								$keys = array('24h' => __('Today', 'twodayssss'), '7d' => __('Week', 'twodayssss'), '30d' => __('Month', 'twodayssss'));
								...
								<tfoot>
									<tr>
										<th></th>
										<?php foreach ($d->localBlocks as $row): ?>
											<td<?php if ($row['type'] == wfActivityReport::BLOCK_TYPE_BLACKLIST && !wfConfig::get('isPaid')) { echo ' class="wf-premium"'; } ?>><?php if ($row['type'] == wfActivityReport::BLOCK_TYPE_BLACKLIST && !wfConfig::get('isPaid')) { _e('Premium', 'twodayssss'); } ?></td>
										<?php endforeach; ?>
										<td></td>
									</tr>
								</tfoot>
							</table>
							<p class="wf-right wf-no-top"><a href="<?php echo wfSupportController::esc_supportURL(wfSupportController::ITEM_WIDGET_LOCAL_ATTACKS); ?>" target="_blank" rel="noopener noreferrer"><i class="wf-fa wf-fa-question-circle-o" aria-hidden="true"></i> <?php _e('How are these categorized?', 'twodayssss'); ?></a></p>
						<?php endif; ?>
					</li>
				</ul>
				<?php endif; ?>
