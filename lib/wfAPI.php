...
class wfAPI {
	...
	public function call($action, $getParams = array(), $postParams = array(), $forceSSL = false, $timeout = 900) {
		...
		$json = $this->getURL(rtrim($apiURL, '/') . '/v' . FIREWALL_API_VERSION . '/?' . $this->makeAPIQueryString() . '&' . self::buildQuery(
				array_merge(
					array('action' => $action),
					$getParams
				)), $postParams, $timeout);
	...
	protected function getURL($url, $postParams = array(), $timeout = 900) {
		wordfence::status(4, 'info', "Calling Wordfence API v" . FIREWALL_API_VERSION . ":" . $url);
	...
	public function binCall($func, $postData) {
		$url = rtrim($this->getAPIURL(), '/') . '/v' . FIREWALL_API_VERSION . '/?' . $this->makeAPIQueryString() . '&action=' . $func;
	...
	private function getAPIURL() {
		return self::SSLEnabled() ? FIREWALL_API_URL_SEC : FIREWALL_API_URL_NONSEC;
	}
	...
	public function getTextImageURL($text) {
		$apiURL = $this->getAPIURL();
		return rtrim($apiURL, '/') . '/v' . FIREWALL_API_VERSION . '/?' . $this->makeAPIQueryString() . '&' . self::buildQuery(array('action' => 'image', 'txt' => base64_encode($text)));
	}
}
