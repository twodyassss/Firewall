...
class wfUtils {
	...
	public static function versionedAsset($subpath) {
		...
		if ($version != 'TF_BUILD_NUMBER' && preg_match('/^(.+?)(\.[^\.]+)$/', $subpath, $matches)) {
			$prefix = $matches[1];
			$suffix = $matches[2];
	...
	public static function getWPVersion($forceRecheck = false){
		...
		if(wordfence::$FIREWALL_wp_version){
			return wordfence::$FIREWALL_wp_version;
		} else {
			global $wp_version;
			return $wp_version;
		}
	...
	public static function getScanLock(){
		...
		if($scanRunning && time() - $scanRunning < FIREWALL_MAX_SCAN_LOCK_TIME){
			return false;
		}
	...
	public static function getIPsGeo($IPs){ //works with int or dotted. Outputs same format it receives.
		...
		foreach($IPs as $IP){
			...
			if($row){
				if($row['age'] > FIREWALL_MAX_IPLOC_AGE){
	...
	public static function reverseLookup($IP) {
		...
		$host = $db->querySingle("select host from " . $reverseTable . " where IP=%s and unix_timestamp() - lastUpdate < %d", $IPn, FIREWALL_REVERSE_LOOKUP_CACHE_TIME);
		if (!$host) {
	...
	public static function fileTooBig($file){ //Deals with files > 2 gigs on 32 bit systems which are reported with the wrong size due to integer overflow
		...
		wfUtils::errorsOn();
		if(! $fh){ return false; }
		$offset = FIREWALL_MAX_FILE_SIZE_TO_PROCESS + 1;
		$tooBig = false;
		try {
	...
	private static function _home_url_nofilter($path = '', $scheme = null) { //A version of the native get_home_url and get_option without the filter calls
		...
		if (defined('WP_HOME') && FIREWALL_PREFER_WP_HOME_FOR_WPML) {
			$cached_url = WP_HOME;
			return $cached_url;
		}
	...
	private static function _site_url_nofilter($path = '', $scheme = null) { //A version of the native get_site_url and get_option without the filter calls
		...
		if (defined('WP_SITEURL') && FIREWALL_PREFER_WP_HOME_FOR_WPML) {
