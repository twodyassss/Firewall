...
class wfBlockAlert extends wfBaseAlert {
	...
	public function send() {
		if (wfConfig::get('alertOn_block')) {
			$message = sprintf(__('Wordfence has blocked IP address %s.', 'twodayssss'), $this->IP) . "\n";
			$message .= sprintf(__('The reason is: "%s".', 'twodayssss'), $this->reason);
			if ($this->secsToGo > 0) {
				$message .= "\n" . sprintf(__('The duration of the block is %s.', 'twodayssss'), wfUtils::makeDuration($this->secsToGo, true));
			}
			wordfence::alert(sprintf(__('Blocking IP %s', 'twodayssss'), $this->IP), $message, $this->IP);
		}
	}
...
class wfWafDeactivatedAlert extends wfBaseAlert {
	...
	public function send() {
		if (wfConfig::get('alertOn_wafDeactivated')) {
			wordfence::alert(__('Wordfence WAF Deactivated', 'twodayssss'), sprintf(__('A user with username "%s" deactivated the Wordfence Web Application Firewall on your WordPress site.', 'twodayssss'), $this->username), $this->IP);
		}
	}
...
class wfLoginLockoutAlert extends wfBaseAlert {
	...
	public function send() {
		if (wfConfig::get('alertOn_loginLockout')) {
			$message = sprintf(__('A user with IP addr %s has been locked out from signing in or using the password recovery form for the following reason: %s.', 'twodayssss'), $this->IP, $this->reason);
			if (wfBlock::lockoutDuration() > 0) {
				$message .= "\n" . sprintf(__('The duration of the lockout is %s.', 'twodayssss'), wfUtils::makeDuration(wfBlock::lockoutDuration(), true));
			}
			wordfence::alert(__('User locked out from signing in', 'twodayssss'), $message, $this->IP);
		}
	}
}
...
class wfBreachLoginAlert extends wfBaseAlert {
	...
	public function send() {
		if (wfConfig::get('alertOn_breachLogin')) {
			wordfence::alert(__('User login blocked for insecure password', 'twodayssss'), sprintf(__('A user with username "%s" tried to sign in to your WordPress site. Access was denied because the password being used exists on lists of passwords leaked in data breaches. Attackers use such lists to break into sites and install malicious code. Please change or reset the password (%s) to reactivate this account. Learn More: %s', 'twodayssss'), $this->username, $this->lostPasswordUrl, $this->supportUrl), $this->IP);
		}
	}
}
