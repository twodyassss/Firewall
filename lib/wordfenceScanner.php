...
class wordfenceScanner {
	...
	protected function setupSigs() {
		...
		if(! (is_array($sigData) && isset($sigData['rules'])) ){
			throw new Exception(__('Wordfence could not get the attack signature patterns from the scanning server.', 'twodayssss'));
		}
		...
		if (is_array($sigData['rules'])) {
			...
			foreach ($sigData['rules'] as $key => $signatureRow) {
				...
				if (empty($pattern)) {
					throw new Exception(__('Wordfence received malformed attack signature patterns from the scanning server.', 'twodayssss'));
				}
				...
				if (@preg_match('/' . $pattern . '/iS', null) === false) {
					wordfence::status(1, 'error', __('A regex Wordfence received from its servers is invalid. The pattern is: ', 'twodayssss') . esc_html($pattern));
	...
	public function scan($forkObj){
		...
		if (is_numeric($backtrackLimit)) {
			...
			if ($backtrackLimit > 10000000) {
				...
				wordfence::status(4, 'info', sprintf(__('Backtrack limit is %d, reducing to 1000000', 'twodayssss'), $backtrackLimit));
			}
		}
		else {
		...
		while (true) {
			...
			if ($thisCount == $lastCount) {
				...
				wordfence::status(4, 'info', __('Detected loop in malware scan, aborting.', 'twodayssss'));
				break;
			}
			$lastCount = $thisCount;
			...
			if (count($files) < 1) {
				wordfence::status(4, 'info', __('No files remaining for malware scan.', 'twodayssss'));
				break;
			}
			...
			foreach ($files as $record) {
				...
				if(wfUtils::fileTooBig($this->path . $file)){ //We can't use filesize on 32 bit systems for files > 2 gigs
					...
					wordfence::status(2, 'error', sprintf(__('Encountered file that is too large: %s - Skipping.', 'twodayssss'), $file));
					$record->markComplete();
					continue;
				}
				...
				if (function_exists('memory_get_usage')) {
					wordfence::status(4, 'info', sprintf(__('Scanning contents: %s (Size: %s Mem: %s)', 'twodayssss'), $file, $fsize, wfUtils::formatBytes(memory_get_usage(true))));
				} else {
					wordfence::status(4, 'info', sprintf(__('Scanning contents: %s (Size: %s)', 'twodayssss'), $file, $fsize));
				}
				...
				while (!feof($fh)) {
					...
					$extraMsg = '';
					if ($isScanImagesFile) {
						$extraMsg = ' ' . __('This file was detected because you have enabled "Scan images, binary, and other files as if they were executable", which treats non-PHP files as if they were PHP code. This option is more aggressive than the usual scans, and may cause false positives.', 'twodayssss');
					}
					else if ($isHighSensitivityFile) {
						$extraMsg = ' ' . __('This file was detected because you have enabled HIGH SENSITIVITY scanning. This option is more aggressive than the usual scans, and may cause false positives.', 'twodayssss');
					}
					...
					if ($options['scansEnabled_fileContents']) {
						if ($treatAsBinary && wfUtils::strpos($data, '$allowed'.'Sites') !== false && wfUtils::strpos($data, "define ('VER"."SION', '1.") !== false && wfUtils::strpos($data, "TimThum"."b script created by") !== false) {
							$this->addResult(array(
								...
								'ignoreC' => $fileSum,
								'shortMsg' => __('File is an old version of TimThumb which is vulnerable.', 'twodayssss'),
								'longMsg' => __('This file appears to be an old version of the TimThumb script which makes your system vulnerable to attackers. Please upgrade the theme or plugin that uses this or remove it.', 'twodayssss') . $extraMsg,
							...
							foreach ($this->patterns['rules'] as $rule) {
								...
								if (!empty($stoppedOnSignature)) { //Advance until we find the rule we stopped on last time
									...
									if ($stoppedOnSignature == $rule[0]) {
										...
										wordfence::status(4, 'info', sprintf(__('Resuming malware scan at rule %s.', 'twodayssss'), $rule[0]));
									}
									continue;
								}
								...
								$customMessage = isset($rule[9]) ? $rule[9] : __('This file appears to be installed or modified by a hacker to perform malicious activity. If you know about this file you can choose to ignore it to exclude it from future scans.', 'twodayssss');
								...
								if (preg_match('/(' . $rule[2] . ')/iS', $data, $matches, PREG_OFFSET_CAPTURE)) {
									...
									if (!$logOnly) {
										$this->addResult(array(
											...
											'ignoreP' => $this->path . $file,
											'ignoreC' => $fileSum,
											'shortMsg' => __('File appears to be malicious: ', 'twodayssss') . esc_html($file),
											'longMsg' => $customMessage . ' ' . __('The matched text in this file is:', 'twodayssss') . ' ' . '<strong style="color: #F00;" class="wf-split-word">' . wfUtils::potentialBinaryStringToHTML((wfUtils::strlen($matchString) > 200 ? wfUtils::substr($matchString, 0, 200) . '...' : $matchString)) . '</strong>' . '<br><br>' . sprintf(__('The issue type is: <strong>%s</strong>', 'twodayssss'), esc_html($rule[7])) . '<br>' . sprintf(__('Description: <strong>%s</strong>', 'twodayssss'), esc_html($rule[3])) . $extraMsg,
											'data' => array_merge(array(
												'file' => $file,
												'shac' => $record->SHAC,
												'highSense' => $options['scansEnabled_highSense'],
								...
								if ($forkObj->shouldFork()) {
									...
									wordfence::status(4, 'info', sprintf(__('Forking during malware scan (%s) to ensure continuity.', 'twodayssss'), $rule[0]));
									$forkObj->fork(); //exits
								}
							}
		...
		$this->writeScanningStatus();
		if ($options['scansEnabled_fileContentsGSB']) {
			wordfence::status(2, 'info', __('Asking Wordfence to check URLs against malware list.', 'twodayssss'));
				...
				foreach($hresults as $result){
					...
					if ($result['badList'] == 'goog-malware-shavar') {
						$this->addResult(array(
							...
							'shortMsg' => __('File contains suspected malware URL: ', 'twodayssss') . esc_html($file),
							'longMsg' => sprintf(__('This file contains a suspected malware URL listed on Google\'s list of malware sites. Wordfence decodes %s when scanning files so the URL may not be visible if you view this file. The URL is: %s - More info available at <a href="http://safebrowsing.clients.google.com/safebrowsing/diagnostic?site=%s&client=googlechrome&hl=en-US" target="_blank" rel="noopener noreferrer">Google Safe Browsing diagnostic page</a>.', 'twodayssss'), esc_html($this->patterns['word3']), esc_html($result['URL']), urlencode($result['URL'])),
							'data' => array_merge(array(
								'file' => $file,
								'shac' => $record->SHAC,
								'badURL' => $result['URL'],
						...
						$this->addResult(array(
							...
							'ignoreP' => $this->path . $file,
							'ignoreC' => md5_file($this->path . $file),
							'shortMsg' => __('File contains suspected phishing URL: ', 'twodayssss') . esc_html($file),
							'longMsg' => __('This file contains a URL that is a suspected phishing site that is currently listed on Google\'s list of known phishing sites. The URL is: ', 'twodayssss') . esc_html($result['URL']),
							'data' => array_merge(array(
								'file' => $file,
								'shac' => $record->SHAC,
								'badURL' => $result['URL'],
						...
						$this->addResult(array(
							...
							'ignoreC' => md5_file($this->path . $file),
							'shortMsg' => __('File contains suspected malware URL: ', 'twodayssss') . esc_html($file),
							'longMsg' => __('This file contains a URL that is currently listed on Wordfence\'s domain blacklist. The URL is: ', 'twodayssss') . esc_html($result['URL']),
							'data' => array_merge(array(
								'file' => $file,
								'shac' => $record->SHAC,
								'badURL' => $result['URL'],
		...
		wordfence::status(4, 'info', __('Finalizing malware scan results', 'twodayssss'));
		$hashesToCheck = array();
	...
	protected function writeScanningStatus() {
		wordfence::status(2, 'info', sprintf(__('Scanned contents of %d additional files at %.2f per second', 'twodayssss'), $this->totalFilesScanned, ($this->totalFilesScanned / (microtime(true) - $this->startTime))));
	}
