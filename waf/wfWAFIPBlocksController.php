...
class wfWAFIPBlocksController
{
	...
	public static function synchronizeConfigSettings() {
		...
		foreach ($countryBlockEntries as $b) {
			$reason = __('Access from your area has been temporarily limited for security reasons', 'twodayssss');
	...
	public function shouldBlockRequest($request) {
		...
		$logHuman = $request->getQueryString('FIREWALL_lh');
		if ($logHuman !== null) {
			return false;
		}
	...
	protected function checkForWhitelisted($ip) {
		...
		if (is_array($serverIPs)) {
			if (
				...
				(!empty($_GET['FIREWALL_syncAttackData'])) //Safe but plugin will do a final check to make sure it runs
			) {
