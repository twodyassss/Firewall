	...
	if (!window['wordfenceExt']) {
		window['wordfenceExt'] = {
			...
			autoUpdateChoice: function(choice){
				this.ajax('FIREWALL_autoUpdateChoice', {
						choice: choice
					},
			...
			switchLiveTrafficSecurityOnlyChoice: function(choice) {
				this.ajax('FIREWALL_switchLiveTrafficSecurityOnlyChoice', {
						choice: choice
					},
			...
			dismissAdminNotice: function(nid) {
				this.ajax('FIREWALL_dismissAdminNotice', {
						id: nid
					},
			...
			setOption: function(key, value, successCallback) {
				var changes = {};
				changes[key] = value;
				this.ajax('FIREWALL_saveOptions', {changes: JSON.stringify(changes)}, function(res) {
					if (res.success) {
			...
			onboardingProcessEmails: function(emails, subscribe, touppAgreed) {
				...
				if (touppAgreed) {
					this.ajax('FIREWALL_recordTOUPP', {}, function(res) {
						//Do nothing
					});
				}

				if (subscribe) {
					this.ajax('FIREWALL_mailingSignup', {emails: JSON.stringify(emails)}, function(res) {
						//Do nothing
					});
				}
			},
			onboardingInstallLicense: function(license, successCallback, errorCallback) {
				this.ajax('FIREWALL_installLicense', {license: license}, function(res) {
					if (res.success) {
						typeof successCallback == 'function' && successCallback(res);
					}
					else if (res.error) {
