		...
		window['wordfenceAdmin'] = {
			...
			init: function() {
				...
				$('.downloadLogFile').each(function() {
					$(this).attr('href', WordfenceAdminVars.ajaxURL + '?action=FIREWALL_downloadLogFile&nonce=' + WFAD.nonce + '&logfile=' + encodeURIComponent($(this).data('logfile')));
				});
				...
				$('#doSendEmail').click(function() {
					...
					WFAD.ajax('FIREWALL_sendDiagnostic', {email: $('#_email').val(), ticket: ticket}, function(res) {
						if (res.result) {
				...
				$('.wf-block-header-action-disclosure').each(function() {
					...
					$(this).closest('.wf-block-header').on('click', function(e) {
						...
						WFAD.ajax('FIREWALL_saveDisclosureState', {name: $(this).closest('.wf-block').data('persistenceKey'), state: !isActive}, function() {}, function() {}, true);
					});
				});
			...
			sendTestEmail: function(email) {
				var self = this;
				this.ajax('FIREWALL_sendTestEmail', {email: email}, function(res) {
					if (res.result) {
			...
			updateConfig: function(key, val, cb) {
				this.ajax('FIREWALL_updateConfig', {key: key, val: val}, function(ret) {
					if (cb) {
						cb(ret);
					}
				});
			...
			updateIPPreview: function(val, cb) {
				this.ajax('FIREWALL_updateIPPreview', val, function(ret) {
					if (cb) {
						cb(ret);
					}
				});
			...
			tourFinish: function(page) {
				...
				WFAD.ajax('FIREWALL_tourClosed', {page: page}, function(res) {});
			},
			...
			updateActivityLog: function() {
				...
				var self = this;
				this.ajax('FIREWALL_activityLogUpdate', {
			...
			updateLiveTraffic: function(forceUpdate) {
				...
				if (this.mode == 'liveTraffic') {
					...
					this.ajax('FIREWALL_ticker', data, function(res) {
			...
			avatarLookup: function() {
				...
				this.ajax('FIREWALL_avatarLookup', {
						ids: uniqueIDs.join(',')
					},
					function(res) {
						if (res.ok) {
			...
			reverseLookupIPs: function() {
				...
				this.ajax('FIREWALL_reverseLookup', {
						ips: uniqueIPs.join(',')
					},
					function(res) {
						if (res.ok) {
			...
			killScan: function(callback) {
				this.ajax('FIREWALL_killScan', {}, function(res) {
					if (res.ok) {
			...
			startScan: function() {
				this.ajax('FIREWALL_scan', {}, function(res) {
					if (res.ok) {
			...
			loadIssues: function(callback, offset, limit) {
				...
				var self = this;
				this.ajax('FIREWALL_loadIssues', {offset: offset, limit: limit}, function(res) {
			...
			loadMoreIssues: function(callback, offset, limit, ignoredOffset, ignoredLimit) {
				...
				WFAD.ajax('FIREWALL_loadIssues', {offset: offset, limit: limit, ignoredOffset: ignoredOffset, ignoredLimit: ignoredLimit}, function(res) {
					WFAD.updateIssueCounts(res.issueCounts);
			...
			bulkOperationConfirmed: function(op) {
				WFAD.colorboxClose();
				this.ajax('FIREWALL_bulkOperation', {
					op: op
				}, function(res) {
					if (res.ok) {
			...
			deleteDatabaseOption: function(issueID) {
				var self = this;
				this.ajax('FIREWALL_deleteDatabaseOption', {
					issueID: issueID
				}, function(res) {
			...
			useRecommendedHowGetIPs: function(issueID) {
				var self = this;
				this.ajax('FIREWALL_misconfiguredHowGetIPsChoice', {
					issueID: issueID,
					choice: 'yes'
				}, function(res) {
					if (res.ok) {
			...
			fixFPD: function(issueID) {
				...
				this.ajax('FIREWALL_checkHtaccess', {}, function(res) {
					if (res.ok) {
						self.colorboxModalHTML((self.isSmallScreen ? '300px' : '400px'), title, 'We are about to change your <em>.htaccess</em> file. Please make a backup of this file before proceeding.'
							+ '<br/>'
							+ '<a href="' + WordfenceAdminVars.ajaxURL + '?action=FIREWALL_downloadHtaccess&nonce=' + self.nonce + '" onclick="jQuery(\'#wfFPDNextBut\').prop(\'disabled\', false); return true;">Click here to download a backup copy of your .htaccess file now</a><br /><br /><input type="button" class="wf-btn wf-btn-default" name="but1" id="wfFPDNextBut" value="Click to fix .htaccess" disabled="disabled" onclick="WFAD.fixFPD_WriteHtAccess(' + issueID + ');" />');
					} else if (res.nginx) {
			...
			hideFile: function(issueID, callback) {
				WFAD.ajax('FIREWALL_checkHtaccess', {}, function(checkRes) {
					if (checkRes.ok) {
						WFAD.colorboxModalHTML((WFAD.isSmallScreen ? '300px' : '400px'), '.htaccess change', 'We are about to change your <em>.htaccess</em> file. Please make a backup of this file before proceeding.'
							+ '<br/>'
							+ '<a id="dlButton" href="' + WordfenceAdminVars.ajaxURL + '?action=FIREWALL_downloadHtaccess&nonce=' + WFAD.nonce + '">Click here to download a backup copy of your .htaccess file now</a>'
						...
						$('#wfFPDNextBut').on('click', function(e) {
							...
							WFAD.ajax('FIREWALL_hideFileHtaccess', {
								issueID: issueID
							}, function(res) {
								WFAD.colorboxClose();
								typeof callback === 'function' && callback(res);
			...
			restoreFile: function(issueID, callback) {
				this.ajax('FIREWALL_restoreFile', {
					issueID: issueID
				}, function(res) {
					if (res.needsCredentials) {
						document.location.href = res.redirect;
			...
			disableDirectoryListing: function(issueID) {
				...
				this.ajax('FIREWALL_checkHtaccess', {}, function(res) {
					if (res.ok) {
						self.colorboxModalHTML((self.isSmallScreen ? '300px' : '400px'), title, 'We are about to change your <em>.htaccess</em> file. Please make a backup of this file before proceeding.'
							+ '<br/>'
							+ '<a href="' + WordfenceAdminVars.ajaxURL + '?action=FIREWALL_downloadHtaccess&nonce=' + self.nonce + '" onclick="jQuery(\'#wf-htaccess-confirm\').prop(\'disabled\', false); return true;">Click here to download a backup copy of your .htaccess file now</a>' +
							'<br /><br />' +
			...
			confirmDisableDirectoryListing: function(issueID) {
				var self = this;
				this.colorboxClose();
				this.ajax('FIREWALL_disableDirectoryListing', {
					issueID: issueID
				}, function(res) {
					if (res.ok) {
						self.loadIssues(function() {
			...
			deleteIssue: function(id) {
				var self = this;
				this.ajax('FIREWALL_deleteIssue', {id: id}, function(res) {
					self.loadIssues();
				});
			},
			updateIssueStatus: function(id, st, callback) {
				this.ajax('FIREWALL_updateIssueStatus', {id: id, 'status': st}, function(res) {
					typeof callback === 'function' && callback(res);
				});
			},
			es: function(val) {
			...
			completeLiveTabSwitch: function() {
				this.ajax('FIREWALL_loadActivityLog', {}, function(res) {
			...
			completeEmailActivityLog: function() {
				...
				var self = this;
				this.ajax('FIREWALL_sendActivityLog', {email: jQuery('#wfALogRecip').val()}, function(res) {
					if (res.ok) {
			...
			blockIPUARange: function(ipRange, hostname, uaRange, referer, reason) {
				...
				var self = this;
				this.ajax('FIREWALL_blockIPUARange', {
					ipRange: ipRange,
					hostname: hostname,
					uaRange: uaRange,
					referer: referer,
			...
			unlockOutIP: function(IP) {
				var self = this;
				this.ajax('FIREWALL_unlockOutIP', {
					IP: IP
				}, function(res) {
			...
			unblockNetwork: function(id) {
				var self = this;
				this.ajax('FIREWALL_unblockRange', {
					id: id
				}, function(res) {
					self.reloadLiveTraffic();
				});
			...
			permBlockIP: function(IP) {
				var self = this;
				this.ajax('FIREWALL_permBlockIP', {
					IP: IP
				}, function(res) {
					self.staticTabChanged();
				});
			...
			addTwoFactor: function(username, phone, mode) {
				var self = this;
				this.ajax('FIREWALL_addTwoFactor', {
					username: username,
					phone: phone,
					mode: mode
				}, function(res) {
			...
			twoFacActivate: function(userID, code) {
				var self = this;
				this.ajax('FIREWALL_twoFacActivate', {
					userID: userID,
					code: code
				}, function(res) {
					if (res.ok) {
			...
			loadTwoFactor: function() {
				this.ajax('FIREWALL_loadTwoFactor', {}, function(res) {
			...
			deleteAdminUser: function(issueID) {
				var self = this;
				this.ajax('FIREWALL_deleteAdminUser', {
					issueID: issueID
				}, function(res) {
					if (res.ok) {
						self.loadIssues(function() {
			...
			revokeAdminUser: function(issueID) {
				var self = this;
				this.ajax('FIREWALL_revokeAdminUser', {
					issueID: issueID
				}, function(res) {
					if (res.ok) {
						self.loadIssues(function() {
			...
			permanentlyBlockAllIPs: function(type) {
				var self = this;
				this.ajax('FIREWALL_permanentlyBlockAllIPs', {
					type: type
				}, function(res) {
			...
			wafUpdateRules: function(onSuccess) {
				var self = this;
				this.ajax('FIREWALL_updateWAFRules', {}, function(res) {
					self.wafData = res;
			...
			confirmWAFConfigureAutoPrepend: function() {
				var self = this;
				this.ajax('FIREWALL_wafConfigureAutoPrepend', {}, function(res) {
			...
			setOption: function(key, value, successCallback, failureCallback) {
				var changes = {};
				changes[key] = value;
				this.ajax('FIREWALL_saveOptions', {changes: JSON.stringify(changes), page: WFAD.getParameterByName('page')}, function(res) {
					if (res.success) {
			...
			saveOptions: function(successCallback, failureCallback) {
				...
				this.ajax('FIREWALL_saveOptions', {changes: JSON.stringify(WFAD.pendingChanges), page: WFAD.getParameterByName('page')}, function(res) {
					if (res.success) {
						typeof successCallback == 'function' && successCallback(res); 
					}
					else {
			...
			enableAllOptionsPage: function() {
				this.ajax('FIREWALL_enableAllOptionsPage', {}, function(res) {
					if (res.redirect) {
		...
		$('#wf-restore-defaults').on('click', function(e) {
			...
			WFAD.colorboxHTML((WFAD.isSmallScreen ? '300px' : '400px'), promptHTML, {overlayClose: false, closeButton: false, className: 'wf-modal', onComplete: function() {
				...
				$('#wf-restore-defaults-prompt-confirm').on('click', function(e) {
					...
					WFAD.ajax('FIREWALL_restoreDefaults', {section: restoreDefaultsSection}, function(res) {
						if (res.success) {
							window.location.reload(true);
						}
						else {
