			...
			<div class="wf-block-header">
				<div class="wf-block-header-content">
					<div class="wf-block-title">
						<strong><?php _e('Advanced Firewall Options', 'twodayssss'); ?></strong>
					</div>
				...
				<ul class="wf-block-list">
					<li>
						...
						echo wfView::create('options/option-toggled', array(
							...
							'title' => __('Delay IP and Country blocking until after WordPress and plugins have loaded (only process firewall rules early)', 'twodayssss'),
							'subtitle' => ($firewall->isSubDirectoryInstallation() ? __('You are currently running the WAF from another WordPress installation. This option can be changed once you configure the firewall to run correctly on this site.', 'twodayssss') : ''),
					...
					<li>
						...
						echo wfView::create('options/option-textarea', array(
							...
							'title' => __('Whitelisted IP addresses that bypass all rules', 'twodayssss'),
							'alignTitle' => 'top',
					...
					<li>
						...
						echo wfView::create('options/option-toggled-multiple', array(
							'options' => $options,
							'title' => __('Whitelisted services', 'twodayssss'),
					...
					<li>
						...
						echo wfView::create('options/option-textarea', array(
							...
							'title' => __('Ignored IP addresses for Wordfence Web Application Firewall alerting', 'twodayssss'),
							'alignTitle' => 'top',
							'subtitle' => __('Ignored IPs must be separated by commas or placed on separate lines. These addresses will be ignored from any alerts about increased attacks and can be used to ignore things like standalone website security scanners.', 'twodayssss'),
							'subtitlePosition' => 'value',
...
<script type="text/x-jquery-template" id="waf-rules-tmpl">
	<table class="wf-striped-table">
		...
		<tr>
			<th style="width: 5%"></th>
			<th><?php _e('Category', 'twodayssss'); ?></th>
			<th><?php _e('Description', 'twodayssss'); ?></th>
		</tr>
		</thead>
		<tbody>
		{{each(idx, rule) rules}}
		...
		{{if (rules.length == 0)}}
		<tr>
			<td colspan="4"><?php _e('No rules currently set.', 'twodayssss'); ?> <?php if (!($firewall->protectionMode() == wfFirewall::PROTECTION_MODE_EXTENDED && $firewall->isSubDirectoryInstallation())) { _e('<a href="#" onclick="WFAD.wafUpdateRules();return false;">Click here</a> to pull down the latest from the Wordfence servers.', 'twodayssss'); } ?>
			</td>
		</tr>
		{{/if}}
		</tbody>
		...
		<tr id="waf-show-all-rules">
			<td class="wf-center" colspan="4"><a href="#" id="waf-show-all-rules-button"><?php _e('SHOW ALL RULES', 'twodayssss'); ?></a></td>
		</tr>
		{{/if}}
		</tfoot>
	</table>
