			...
			<div class="wf-block-header">
				<div class="wf-block-header-content">
					<div class="wf-block-title">
						<strong><?php _e('Whitelisted URLs', 'twodayssss'); ?></strong>
					</div>
							...
							echo wfView::create('options/option-toggled-multiple', array(
								'options' => array(
									array(
										...
										'title' => __('Front-end Website', 'twodayssss'),
									),
									array(
										...
										'title' => __('Admin Panel', 'twodayssss'),
									),
								),
								'noSpacer' => true,
								'htmlTitle' => '<strong>' . __('Monitor background requests from an administrator\'s web browser for false positives', 'twodayssss') . '</strong>',
...
<script type="text/x-jquery-template" id="waf-whitelisted-urls-tmpl">
	<div class="whitelist-table-container">
		<table class="wf-striped-table whitelist-table">
			<thead>
			<tr>
				<th style="width: 2%;text-align: center"><div class="wf-whitelist-bulk-select wf-option-checkbox" role="checkbox" aria-checked="false" tabindex="0" aria-label="<?php esc_attr_e('Select/deselect all', 'twodayssss'); ?>"><i class="wf-ion-ios-checkmark-empty" aria-hidden="true"></i></div></th>
				<th style="width: 5%;"><?php _e('Enabled', 'twodayssss'); ?></th>
				<th><?php _e('URL', 'twodayssss'); ?></th>
				<th><?php _e('Param', 'twodayssss'); ?></th>
				<th><?php _e('Created', 'twodayssss'); ?></th>
				<th><?php _e('Source', 'twodayssss'); ?></th>
				<th><?php _e('User', 'twodayssss'); ?></th>
				<th><?php _e('IP', 'twodayssss'); ?></th>
			</tr>
			</thead>
			...
			<tr data-index="${idx}" data-adding="{{if (whitelistedURLParam.adding)}}1{{else}}0{{/if}}" data-key="${whitelistedURLParam.path}|${whitelistedURLParam.paramKey}">
				<td style="text-align: center;"><div class="wf-whitelist-table-bulk-checkbox wf-option-checkbox" role="checkbox" aria-checked="false" tabindex="0" aria-label="<?php esc_attr_e('Select row ${idx}', 'twodayssss'); ?>"><i class="wf-ion-ios-checkmark-empty" aria-hidden="true"></i></div></td>
				<td style="text-align: center;"><div class="wf-whitelist-item-enabled wf-option-checkbox{{if (!whitelistedURLParam.data.disabled)}} wf-checked{{/if}}" data-original-value="{{if (!whitelistedURLParam.data.disabled)}}1{{else}}0{{/if}}" role="checkbox" aria-checked="{{if (!whitelistedURLParam.data.disabled)}}true{{else}}false{{/if}}" tabindex="0" aria-label="<?php esc_attr_e('Toggle row ${idx}', 'twodayssss'); ?>"><i class="wf-ion-ios-checkmark-empty" aria-hidden="true"></i></div></td>
				<td data-column="url">
			...
			<tr>
				<td colspan="8"><?php _e('No whitelisted URLs currently set.', 'twodayssss'); ?></td>
			</tr>
			{{/if}}
			</tbody>
		</table>
