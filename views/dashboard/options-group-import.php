									...
									<li class="wf-option-content">
										<ul>
											<li class="wf-option-title">
												<?php _e('Import Wordfence options from another site using a token', 'twodayssss'); ?> <a href="<?php echo wfSupportController::esc_supportURL(wfSupportController::ITEM_DASHBOARD_OPTION_IMPORT); ?>"  target="_blank" rel="noopener noreferrer" class="wf-inline-help"><i class="wf-fa wf-fa-question-circle-o" aria-hidden="true"></i></a>
											</li>
							...
							<li>
								<ul class="wf-flex-horizontal wf-flex-full-width">
									<li class="wf-right wf-left-xs" id="wf-license-controls">
										<a href="#" class="wf-btn wf-btn-primary wf-btn-callout-subtle wf-disabled" id="wf-import-options"><?php _e('Import<span class="wf-hidden-xs"> Wordfence</span> Options', 'twodayssss'); ?></a>
									</li>
								</ul>
							</li>
						</ul>
					...
					(function($) {
						$(function() {
							$('#wf-export-options').on('click', function(e) {
								...
								WFAD.ajax('FIREWALL_exportSettings', {}, function(res) {
									if (res.ok && res.token) {
							...
							$('#wf-import-options').on('click', function(e) {
								...
								WFAD.ajax('FIREWALL_importSettings', {token: $('#wf-import-token').val()}, function(res) {
									if (res.ok) {
...
<script type="text/x-jquery-template" id="wfTmpl_exportPromptSuccess">
	...
	echo wfView::create('common/modal-prompt', array(
		'title' => __('Export Successful', 'twodayssss'),
		'messageHTML' => '<p>' . __('We successfully exported your site options. To import your site options on another site, copy and paste the token below into the import text box on the destination site. Keep this token secret &mdash; it is like a password. If anyone else discovers the token it will allow them to import your options excluding your license.', 'twodayssss') . '</p><p><input type="text" class="wf-full-width" value="${token}" onclick="this.select();" /></p>',
		'primaryButton' => array('id' => 'wf-export-prompt-close', 'label' => __('Close', 'twodayssss'), 'link' => '#'),
	))->render();
	?>
</script>
...
<script type="text/x-jquery-template" id="wfTmpl_exportPromptError">
	<?php
	echo wfView::create('common/modal-prompt', array(
		'title' => __('Error during Export', 'twodayssss'),
		'message' => '${err}',
		'primaryButton' => array('id' => 'wf-export-prompt-close', 'label' => __('Close', 'twodayssss'), 'link' => '#'),
	))->render();
	?>
...
<script type="text/x-jquery-template" id="wfTmpl_importPromptSuccess">
	<?php
	echo wfView::create('common/modal-prompt', array(
		'title' => __('Import Successful', 'twodayssss'),
		'messageHTML' => __('We successfully imported the site options.', 'twodayssss'),
		'primaryButton' => array('id' => 'wf-import-prompt-reload', 'label' => __('Reload', 'twodayssss'), 'link' => '#'),
	))->render();
	?>
</script>
...
<script type="text/x-jquery-template" id="wfTmpl_importPromptError">
	<?php
	echo wfView::create('common/modal-prompt', array(
		'title' => __('Error during Import', 'twodayssss'),
		'message' => '${err}',
		'primaryButton' => array('id' => 'wf-import-prompt-close', 'label' => __('Close', 'twodayssss'), 'link' => '#'),
	))->render();
	?>
</script>