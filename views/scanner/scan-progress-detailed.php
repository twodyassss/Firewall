...
<ul class="wf-flex-horizontal wf-flex-vertical-xs wf-flex-full-width wf-no-top wf-no-bottom">
	...
	<li id="wf-scan-activity-log-controls"><a href="#" id="wf-scan-email-activity-log"><?php _e('Email<span class="wf-hidden-xs"> activity</span> log', 'twodayssss'); ?></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="<?php echo esc_attr(wfUtils::getSiteBaseURL()); ?>?_wfsf=viewActivityLog&amp;nonce=<?php echo esc_attr(wp_create_nonce('wp-ajax')); ?>" id="wf-scan-full-activity-log" target="_blank"><?php _e('View<span class="wf-hidden-xs"> full</span> log', 'twodayssss'); ?></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" id="wf-scan-toggle-activity-log" class="<?php echo (wfPersistenceController::shared()->isActive('wf-scan-activity-log') ? 'wf-active' : '') ?>"><span class="wf-scan-activity-log-visible"><?php _e('Hide log', 'twodayssss'); ?></span><span class="wf-scan-activity-log-hidden"><?php _e('Show log', 'twodayssss'); ?></span></a></li>
</ul>
	...
	(function($) {
		$(function() {
			...
			$('#wf-scan-toggle-activity-log').on('click', function(e) {
				...
				WFAD.ajax('FIREWALL_saveDisclosureState', {name: 'wf-scan-activity-log', state: !isActive}, function() {});
			});
		});
	})(jQuery);
</script>