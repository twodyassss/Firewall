...
<div id="wf-scan-starter" class="wf-block-navigation-option">
	<div class="wf-block-navigation-option-content">
		<a href="#" class="wf-btn wf-btn-primary wf-btn-callout-subtle wf-scan-starter-idle" style="<?php if ($running) { echo 'display: none;'; } ?>"><?php _e('Start New Scan', 'twodayssss'); ?></a>
		<a href="#" class="wf-btn wf-btn-primary wf-btn-callout-subtle wf-scan-starter-running" style="<?php if (!$running) { echo 'display: none;'; } ?>;"><?php _e('Stop Scan', 'twodayssss'); ?></a> 
	</div>
</div>
		...
		$('#wf-scan-starter a').on('click', function(e) {
			...
			if ($(this).hasClass('wf-scan-starter-idle')) {
				...
				WFAD.killScan(function(success) {
					WFAD.colorboxModal((WFAD.isSmallScreen ? '300px' : '400px'), success ? '<?php esc_attr_e('Scan Stopping', 'twodayssss'); ?>' : '<?php esc_attr_e('Stop Failed', 'twodayssss'); ?>', success ? '<?php esc_attr_e('A termination request has been sent to stop any running scans.', 'twodayssss'); ?>' : '<?php esc_attr_e('We failed to send a termination request.', 'twodayssss'); ?>');
				});
