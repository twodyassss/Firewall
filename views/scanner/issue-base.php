...
<ul class="wf-issue wf-issue-<?php echo $internalType; ?>
	...
	<li class="wf-issue-summary">
		<ul>
			...
			<li class="wf-issue-short wf-hidden-xs"><div class="wf-issue-message">${shortMsg}</div><div class="wf-issue-type"><?php echo __('Type:', 'twodayssss') . ' ' . $displayType; ?></div></li>
			<li class="wf-issue-stats wf-hidden-xs">
				<div class="wf-issue-time"><?php _e('Issue Found ', 'twodayssss'); ?> ${displayTime}</div>
				{{if severity == <?php echo wfIssues::SEVERITY_CRITICAL ?>}}<div class="wf-issue-severity-critical"><?php echo __('Critical', 'twodayssss'); ?></div>{{/if}}
				{{if severity == <?php echo wfIssues::SEVERITY_HIGH ?>}}<div class="wf-issue-severity-high"><?php echo __('High', 'twodayssss'); ?></div>{{/if}}
				{{if severity == <?php echo wfIssues::SEVERITY_MEDIUM ?>}}<div class="wf-issue-severity-medium"><?php echo __('Medium', 'twodayssss'); ?></div>{{/if}}
				{{if severity == <?php echo wfIssues::SEVERITY_LOW ?>}}<div class="wf-issue-severity-low"><?php echo __('Low', 'twodayssss'); ?></div>{{/if}}
			</li>
			<li class="wf-issue-short-stats wf-hidden-sm wf-hidden-md wf-hidden-lg">
				...
				<div class="wf-issue-type"><?php echo __('Type:', 'twodayssss') . ' ' . $displayType; ?></div>
				<div class="wf-issue-time"><?php _e('Found ', 'twodayssss'); ?> ${displayTime}</div>
				{{if severity == <?php echo wfIssues::SEVERITY_CRITICAL ?>}}<div class="wf-issue-severity-critical"><?php echo __('Critical', 'twodayssss'); ?></div>{{/if}}
				{{if severity == <?php echo wfIssues::SEVERITY_HIGH ?>}}<div class="wf-issue-severity-high"><?php echo __('High', 'twodayssss'); ?></div>{{/if}}
				{{if severity == <?php echo wfIssues::SEVERITY_MEDIUM ?>}}<div class="wf-issue-severity-medium"><?php echo __('Medium', 'twodayssss'); ?></div>{{/if}}
				{{if severity == <?php echo wfIssues::SEVERITY_LOW ?>}}<div class="wf-issue-severity-low"><?php echo __('Low', 'twodayssss'); ?></div>{{/if}}
	...
	<li class="wf-issue-detail">
		<ul>
			<!--<li><strong><?php _e('Status', 'twodayssss'); ?>: </strong>{{if status == 'new' }}<?php _e('New', 'twodayssss'); ?>{{/if}}{{if status == 'ignoreP' || status == 'ignoreC' }}<?php _e('Ignored', 'twodayssss'); ?>{{/if}}</li>
			<li><strong><?php _e('Issue First Detected', 'twodayssss'); ?>: </strong>${timeAgo} <?php _e('ago', 'twodayssss'); ?>.</li>-->
		<?php
...
<?php else: ?>
<?php
echo '[' . $displayType . ($textOutput['status'] == 'ignoreP' || $textOutput['status'] == 'ignoreP' ? ', ' . __('Ignored', 'twodayssss') : '') . ']' . "\n";
echo $textOutput['shortMsg'] . "\n";
echo sprintf(__('Issue Found: %s', 'twodayssss'), $textOutput['displayTime']) . "\n";
$severity = null;
switch ($textOutput['severity']) {
	case wfIssues::SEVERITY_CRITICAL:
		$severity = __('Critical', 'twodayssss');
		break;
	case wfIssues::SEVERITY_HIGH:
		$severity = __('High', 'twodayssss');
		break;
	case wfIssues::SEVERITY_MEDIUM:
		$severity = __('Medium', 'twodayssss');
		break;
	case wfIssues::SEVERITY_LOW:
		$severity = __('Low', 'twodayssss');
		break;
	default:
		$severity = __('None', 'twodayssss');
		break;
}
if ($severity) {
	echo sprintf(__('Severity: %s', 'twodayssss'), $severity) . "\n";
}
