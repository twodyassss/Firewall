...
class wfBlock {
	...
	public static function nameForType($type) {
		switch ($type) {
			...
			case self::TYPE_RATE_BLOCK:
				return __('IP Block', 'twodayssss');
			case self::TYPE_RATE_THROTTLE:
				return __('IP Throttled', 'twodayssss');
			case self::TYPE_LOCKOUT:
				return __('Lockout', 'twodayssss');
			case self::TYPE_COUNTRY:
				return __('Country Block', 'twodayssss');
			case self::TYPE_PATTERN:
				return __('Advanced Block', 'twodayssss');
		}
		
		return __('Unknown', 'twodayssss');
	}
	...
	public static function isWhitelisted($IP, &$forcedWhitelistEntry = null) {
		...
		if (
			...
			(defined('FIREWALL_SYNCING_ATTACK_DATA') && FIREWALL_SYNCING_ATTACK_DATA) //Safe as long as it will actually run since it then exits
		) {
	...
	public static function validate($payload) {
		if (!isset($payload['type']) || array_search($payload['type'], array('ip-address', 'country', 'custom-pattern')) === false) { return __('Invalid block type.', 'twodayssss'); }
		if (!isset($payload['duration']) || intval($payload['duration']) < 0) { return __('Invalid block duration.', 'twodayssss'); }
		if (!isset($payload['reason']) || empty($payload['reason'])) { return __('A block reason must be provided.', 'twodayssss'); }
		
		if ($payload['type'] == 'ip-address') {
			if (!isset($payload['ip']) || !filter_var(trim($payload['ip']), FILTER_VALIDATE_IP) || @wfUtils::inet_pton(trim($payload['ip'])) === false) { return __('Invalid IP address.', 'twodayssss'); }
			if (self::isWhitelisted(trim($payload['ip']))) { return sprintf(__('This IP address is in a range of addresses that Wordfence does not block. The IP range may be internal or belong to a service that is always allowed. Whitelisting of external services can be disabled. <a href="%s" target="_blank" rel="noopener noreferrer">Learn More</a>', 'twodayssss'), wfSupportController::supportURL(wfSupportController::ITEM_FIREWALL_WAF_OPTION_WHITELISTED_SERVICES)); }
		}
		else if ($payload['type'] == 'country') {
			if (!isset($payload['blockLogin']) || !isset($payload['blockSite'])) { return __('Nothing selected to block.', 'twodayssss'); }
			if (!$payload['blockLogin'] && !$payload['blockSite']) { return __('Nothing selected to block.', 'twodayssss'); }
			if (!isset($payload['countries']) || empty($payload['countries']) || !is_array($payload['countries'])) { return __('No countries selected.', 'twodayssss'); }
			...
			foreach ($payload['countries'] as $code) {
				if (!isset($wfBulkCountries[$code])) {
					return __('An invalid country was selected.', 'twodayssss');
				}
			}
		}
			...
			if (isset($payload['ipRange']) && !empty($payload['ipRange'])) {
				...
				if ($ipRange->isValidRange()) {
					if ($ipRange->isMixedRange()) {
						return __('Ranges mixing IPv4 and IPv6 addresses are not supported.', 'twodayssss');
					}
					
					$hasOne = true;
				}
				else {
					return __('Invalid IP range.', 'twodayssss');
				}
			}
			if (isset($payload['hostname']) && !empty($payload['hostname'])) {
				if (preg_match('/^[a-z0-9\.\*\-]+$/i', $payload['hostname'])) {
					...
					return __('Invalid hostname.', 'twodayssss');
				}
			}
			...
			if (!$hasOne) { return __('No block parameters provided.', 'twodayssss'); }
		}
...
END AS `detailSort`
 FROM `{$blocksTable}` WHERE ";
	...
	public static function filteredBlocks($prefetch = false, $ofTypes = array(), $offset = 0, $limit = -1, $sortColumn = 'type', $sortDirection = 'ascending', $filter = '') {
		...
		$returnBlocks = array();
		for ($i = 0; true; $i += FIREWALL_BLOCKED_IPS_PER_PAGE) {
			$blocks = wfBlock::allBlocks(true, $ofTypes, $i, FIREWALL_BLOCKED_IPS_PER_PAGE, $sortColumn, $sortDirection);
			if (empty($blocks)) {
				break;
			}
	...
	public function _checkForBlockedCountryFilter($user) {
		...
		$log = wfLog::shared();
		$log->getCurrentRequest()->actionDescription = __('blocked access via country blocking', 'twodayssss');
		wfConfig::inc('totalCountryBlocked');
		wfActivityReport::logBlockedIP(wfUtils::getIP(), null, 'country');
		$log->do503(3600, __('Access from your area has been temporarily limited for security reasons', 'twodayssss')); //exits
	}
