...
class wfScanner {
	...
	public static function displayScanType($type) {
		switch ($type) {
			case self::SCAN_TYPE_QUICK:
				return __('Quick', 'twodayssss');
			case self::SCAN_TYPE_LIMITED:
				return __('Limited', 'twodayssss');
			case self::SCAN_TYPE_HIGH_SENSITIVITY:
				return __('High Sensitivity', 'twodayssss');
			case self::SCAN_TYPE_CUSTOM:
				return __('Custom', 'twodayssss');
			case self::SCAN_TYPE_STANDARD:
			default:
				return __('Standard', 'twodayssss');
		}
	}
	...
	public static function displayScanTypeDetail($type) {
		switch ($type) {
			case self::SCAN_TYPE_QUICK:
			case self::SCAN_TYPE_LIMITED:
				return __('Low resource utilization, limited detection capability', 'twodayssss');
			case self::SCAN_TYPE_HIGH_SENSITIVITY:
				return __('Standard detection capability, chance of false positives', 'twodayssss');
			case self::SCAN_TYPE_CUSTOM:
				return __('Custom scan options selected', 'twodayssss');
			case self::SCAN_TYPE_STANDARD:
			default:
				return __('Standard detection capability', 'twodayssss');
		}
	}
	 ...
	 * under wf_scanRunning is within FIREWALL_MAX_SCAN_LOCK_TIME seconds of now.
	 * 
	 * @return bool
	 */
	public function isRunning() {
		$scanRunning = wfConfig::get('wf_scanRunning');
		return ($scanRunning && time() - $scanRunning < FIREWALL_MAX_SCAN_LOCK_TIME);
	}
	...
	public function scanTypeStatusList() {
		...
		if ($premiumPercentage > 0) {
			...
			$statusList[] = array(
				...
				'title'      => __('Enable Premium Reputation Checks.', 'twodayssss'),
			);
		}
		...
		if ($percentage > 0) {
			...
			$statusList[] = array(
				'percentage' => $subtraction,
				'title' => sprintf(_nx('Enable %d scan option.', 'Enable %d scan options.', $disabledOptionCount,'twodayssss'), number_format_i18n($disabledOptionCount)),
			);
		}
	...
	public function reputationStatusList() {
		...
		$reputationChecks = array(
			'spamvertizeCheck' => __('Enable scan option to check if this website is being "Spamvertised".', 'twodayssss'),
			'checkSpamIP' => __('Enable scan option to check if your website IP is generating spam.', 'twodayssss'),
			'scansEnabled_checkGSB' => __('Enable scan option to check if your website is on a domain blacklist.', 'twodayssss'),
		);
		...
		if (!empty($value)) {
			...
			foreach ($regexs as $r) {
				...
				if (preg_match('/' . $r . '/i', "") !== false) {
					$signatures[] = array($id++, time(), $r, __('User defined scan pattern', 'twodayssss'));
				}
			}
		}
		return $signatures;
	...
	public function scheduleSingleScan($futureTime, $originalTime = false) {
		if (is_main_site()) { // Removed ability to activate on network site in v5.3.12
			...
			wp_schedule_single_event($futureTime, 'FIREWALL_start_scheduled_scan', array((int) $originalTime));
	...
	public function unscheduleAllScans() {
		...
		foreach ($allScansScheduled as $entry) {
			wp_unschedule_event($entry['timestamp'], 'FIREWALL_start_scheduled_scan', $entry['args']);
		}
		wp_clear_scheduled_hook('FIREWALL_start_scheduled_scan');
