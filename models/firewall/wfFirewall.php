...
class wfFirewall
{
	...
	public function displayText($status = null, $protection = null) {
		...
		switch ($status) {
			case self::FIREWALL_MODE_ENABLED:
				$statusText = __('Enabled', 'twodayssss');
				break;
			case self::FIREWALL_MODE_LEARNING:
				$statusText = __('Learning Mode', 'twodayssss');
				break;
			default:
				return __('Disabled', 'twodayssss');
		}
		
		switch ($protection) {
			case self::PROTECTION_MODE_EXTENDED:
				$protectionText = __('Extended Protection', 'twodayssss');
				break;
			default:
				$protectionText = __('Basic Protection', 'twodayssss');
				break;
		}
	...
	public function wafStatusList($section = null) {
		...
		try {
			...
			if (!$wafEnabled) {
				return array(
					array(
						'percentage' => 1.0,
						'title'      => __('Enable firewall.', 'twodayssss'),
					),
				);
			}
			...
			if (!wfConfig::get('isPaid')) {
				$premiumStatus = array(
					'percentage' => 0.30,
					'title'      => __('Enable Premium Rules.', 'twodayssss'),
				);
			}
			...
			if (!$extendedProtection) {
				$statusList[] = array(
					...
					'title' => __('Optimize the Wordfence Firewall.', 'twodayssss'),
				);
			}
		...
		if (!WFWAF_OPERATIONAL) {
			return array(array('percentage' => 1.0, 'title' => __('Repair the Wordfence Firewall configuration.', 'twodayssss')));
		}
	...
	public function ruleStatusDescription($round = false) {
		try {
			...
			return array(
				...
				'title'      => sprintf(_nx('Re-enable %d firewall rule.', 'Re-enable %d firewall rules.', $reenbleCount, 'twodayssss'), number_format_i18n($reenbleCount)),
			);
		}
		catch (Exception $e) {
			//Ignore, return 0%
	...
	public function blacklistStatusDescription() {
		try {
			...
			return array(
				'percentage' => 1.0,
				'title'      => __('Enable Real-Time IP Blacklist.', 'twodayssss'),
			);
		}
		catch (Exception $e) {
			//Ignore, return 0%
	...
	public function bruteForceStatusList() {
		...
		if ($localBruteForceEnabled) {
			if (!$networkBruteForceEnabled) {
				$status[] = array(
					'percentage' => 0.5,
					'title' => __('Enable Real-Time Wordfence Security Network.', 'twodayssss'),
				);
			}
			...
			if (!wfConfig::get('loginSec_maskLoginErrors')) {
				$status[] = array(
					'percentage' => 0.1,
					'title' => __('Enable Mask Login Errors.', 'twodayssss'),
				);
			}
			if (!wfConfig::get('loginSec_blockAdminReg')) {
				$status[] = array(
					'percentage' => 0.1,
					'title' => __('Enable Block Admin Registration.', 'twodayssss'),
				);
			}
			if (!wfConfig::get('loginSec_disableAuthorScan')) {
				$status[] = array(
					'percentage' => 0.1,
					'title' => __('Disable Author Scanning.', 'twodayssss'),
				);
			}
		} else {
			$status[] = array(
				'percentage' => 1.0,
				'title' => __('Enable Brute Force Protection.', 'twodayssss'),
			);
		}
